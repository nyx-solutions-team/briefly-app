...
const PAGE_SIZE = 20;
const QUEUE_CACHE_TTL_MS = 15_000;
...
function readQueueSnapshot(cacheKey: string) {
  const snapshot = queueSnapshotCache.get(cacheKey);
  if (!snapshot) return null;
  if (Date.now() - snapshot.cachedAt > QUEUE_CACHE_TTL_MS) {
    queueSnapshotCache.delete(cacheKey);
    return null;
  }
  return snapshot;
}

function writeQueueSnapshot(cacheKey: string, snapshot: QueueSnapshot) {
  queueSnapshotCache.set(cacheKey, snapshot);
}

function invalidateQueueSnapshots(orgId: string | undefined) {
  if (!orgId) {
    queueSnapshotCache.clear();
    return;
  }
  for (const key of queueSnapshotCache.keys()) {
...
function getStatusCountKey(status: QueueDocStatus): keyof QueueStatusCounts {
  switch (status) {
    case "ready":
      return "needs_review";
    case "error":
...
export default function QueuePage() {
  ...
  const [statusCounts, setStatusCounts] = useState<QueueStatusCounts>({ ...EMPTY_STATUS_COUNTS });
  ...
  const applyQueueSnapshot = useCallback((snapshot: QueueSnapshot) => {
    ...
    setLastUpdatedAt(snapshot.lastUpdatedAtMs ? new Date(snapshot.lastUpdatedAtMs) : null);
  }, []);

  const optimisticallyRemoveDocs = useCallback((docIds: string[]) => {
    if (docIds.length === 0) return;
    const idSet = new Set(docIds);
    const removedItems = items.filter((item) => idSet.has(item.docId) || idSet.has(item.id));
    if (removedItems.length === 0) return;

    setItems((prev) => prev.filter((item) => !idSet.has(item.docId) && !idSet.has(item.id)));
    setTotalItems((prev) => Math.max(0, prev - removedItems.length));
    setStatusCounts((prev) => {
      const next = { ...prev };
      for (const item of removedItems) {
        const key = getStatusCountKey(item.status);
    ...
    invalidateQueueSnapshots(getApiContext().orgId);
  }, [items]);

  const optimisticallyMarkDocsProcessing = useCallback((docIds: string[], note: string) => {
    if (docIds.length === 0) return;
    const idSet = new Set(docIds);
    const affectedItems = items.filter((item) => idSet.has(item.docId) || idSet.has(item.id));
    if (affectedItems.length === 0) return;

    setItems((prev) =>
      prev.map((item) =>
        idSet.has(item.docId) || idSet.has(item.id)
          ? {
              ...item,
              status: "processing",
              progress: 70,
    ...
    setStatusCounts((prev) => {
      ...
      for (const item of affectedItems) {
        const previousKey = getStatusCountKey(item.status);
  ...
  const fetchQueue = useCallback(
    async (
      ...
      let request: Promise<PaginatedResponse> | undefined;
      try {
        ...
        request = queueRequestCache.get(cacheKey);
        if (!request) {
          request = apiFetch<PaginatedResponse>(
        ...
        if (Array.isArray(response)) {
          ...
          writeQueueSnapshot(cacheKey, {
            items: queueDocs,
            totalItems: queueDocs.length,
            totalPages: 1,
          ...
          const nextStatusCounts = response.statusCounts || { ...EMPTY_STATUS_COUNTS };
        ...
        if (orgId) {
          const cacheKey = buildQueueCacheKey(orgId, page, search, filter);
          if (request && queueRequestCache.get(cacheKey) === request) {
    ...
    [applyQueueSnapshot, items.length, toast]
  );
  ...
  const scheduleQueueRefresh = useCallback((delayMs = 0) => {
    if (refreshTimerRef.current) {
      clearTimeout(refreshTimerRef.current);
    }
    refreshTimerRef.current = setTimeout(() => {
      void fetchQueue(false, currentPage, debouncedSearch, statusFilter);
    }, delayMs);
  }, [currentPage, debouncedSearch, fetchQueue, statusFilter]);
  ...
  const handleBulkAccept = async () => {
    ...
    try {
      ...
      optimisticallyRemoveDocs(docIds);
      clearSelection();
  ...
  const handleSingleAccept = async (docId: string) => {
    ...
    try {
      ...
      optimisticallyRemoveDocs([docId]);
      scheduleQueueRefresh();
    } catch (error) {
      toast({
        title: "Accept Failed",
  ...
  const handleVespaRetry = async (docId: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        optimisticallyMarkDocsProcessing([docId], "Retrying Vespa indexing…");
        scheduleQueueRefresh(2000);
      } else {
  ...
  const handleIngestionRetry = async (docId: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        optimisticallyMarkDocsProcessing([docId], "Retrying ingestion…");
        scheduleQueueRefresh(2000);
      } else {
        throw new Error(response.message || "Retry failed");
      }
  ...
  const handleBulkRetry = async () => {
    ...
    try {
      ...
      optimisticallyMarkDocsProcessing(
        [...vespaIds, ...ingestionIds],
        "Retry queued…"
      );
      clearSelection();
      scheduleQueueRefresh(2000);
          ...
          <div className="px-6 py-4">
            <div className="flex items-center justify-between">
                    ...
                    {loading ? (
                      ...
                      `${totalItems} items · ${totalActive} active${
                        refreshing
                          ? " · Refreshing…"
                          : lastUpdatedAt
                            ? ` · Updated ${formatAppDateTime(lastUpdatedAt)}`
                            : ""
                      }`
                    )}
                  </p>
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => fetchQueue(items.length === 0, currentPage, debouncedSearch, statusFilter)}
