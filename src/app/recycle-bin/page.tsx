...
const PAGE_SIZE = 20;
const RECYCLE_BIN_CACHE_TTL_MS = 15_000;
...
const recycleBinSnapshotCache = new Map<string, RecycleBinSnapshot>();
const recycleBinRequestCache = new Map<string, Promise<PaginatedResponse | BinDoc[]>>();

function buildRecycleBinCacheKey(orgId: string, page: number, search: string) {
  return `${orgId}|${page}|${search.trim().toLowerCase()}`;
}

function readRecycleBinSnapshot(cacheKey: string) {
  const snapshot = recycleBinSnapshotCache.get(cacheKey);
  if (!snapshot) return null;
  if (Date.now() - snapshot.cachedAt > RECYCLE_BIN_CACHE_TTL_MS) {
    recycleBinSnapshotCache.delete(cacheKey);
    return null;
  }
  return snapshot;
}

function writeRecycleBinSnapshot(cacheKey: string, snapshot: RecycleBinSnapshot) {
  recycleBinSnapshotCache.set(cacheKey, snapshot);
}

function invalidateRecycleBinSnapshots(orgId: string | undefined) {
  if (!orgId) {
    recycleBinSnapshotCache.clear();
    return;
  }
  for (const key of recycleBinSnapshotCache.keys()) {
    if (key.startsWith(`${orgId}|`)) {
      recycleBinSnapshotCache.delete(key);
    }
  }
}
...
export default function RecycleBinPage() {
  ...
  const applyRecycleBinSnapshot = useCallback((snapshot: RecycleBinSnapshot) => {
    setItems(snapshot.items);
    setTotalItems(snapshot.totalItems);
    setTotalPages(snapshot.totalPages);
  }, []);

  const optimisticallyRemoveDocs = useCallback((ids: string[]) => {
    ...
    setTotalItems((prev) => Math.max(0, prev - removedCount));
    setSelectedIds((prev) => {
    ...
    invalidateRecycleBinSnapshots(getApiContext().orgId);
  }, [items]);
  ...
  const fetchBin = useCallback(
    async (showLoading = true, page = 1, search = "") => {
      ...
      let request: Promise<PaginatedResponse | BinDoc[]> | undefined;
      try {
        ...
        const cacheKey = buildRecycleBinCacheKey(orgId, page, search);
        const cachedSnapshot = readRecycleBinSnapshot(cacheKey);
        if (cachedSnapshot) {
          applyRecycleBinSnapshot(cachedSnapshot);
        }

        shouldBlock = showLoading && !cachedSnapshot && items.length === 0;
        if (shouldBlock) {
          setLoading(true);
          setRefreshing(false);
        } else {
        ...
        request = recycleBinRequestCache.get(cacheKey);
        if (!request) {
          request = apiFetch<PaginatedResponse | BinDoc[]>(
            `/orgs/${orgId}/recycle-bin?page=${page}&limit=${PAGE_SIZE}${searchParam}`,
            { skipCache: true }
          );
          recycleBinRequestCache.set(cacheKey, request);
        }
        ...
        if (Array.isArray(res)) {
          ...
          setTotalPages(1);
          writeRecycleBinSnapshot(cacheKey, {
            items: res || [],
        ...
        if (orgId) {
          const cacheKey = buildRecycleBinCacheKey(orgId, page, search);
          if (request && recycleBinRequestCache.get(cacheKey) === request) {
            recycleBinRequestCache.delete(cacheKey);
          }
        }
        if (shouldBlock) {
          setLoading(false);
    ...
    [applyRecycleBinSnapshot, items.length, toast]
  );

  useEffect(() => {
    if (isAuthenticated && hasAccess) {
      void fetchBin(!fetchedRef.current, currentPage, debouncedSearch);
  ...
  const scheduleRecycleBinRefresh = useCallback((delayMs = 0) => {
    if (refreshTimerRef.current) {
      clearTimeout(refreshTimerRef.current);
    }
    refreshTimerRef.current = setTimeout(() => {
      void fetchBin(false, currentPage, debouncedSearch);
    }, delayMs);
  }, [currentPage, debouncedSearch, fetchBin]);
  ...
  const handleRestore = async (id: string) => {
    ...
    try {
      ...
      optimisticallyRemoveDocs([id]);
      scheduleRecycleBinRefresh();
  ...
  const executeDelete = async () => {
    ...
    if (type === "bulk") setBulkDeleting(true);
    else if (id) setDeleteLoadingId(id);
    ...
    try {
      ...
      if (id) setDeleteLoadingId((current) => (current === id ? null : current));
    }
  };

  const isDeleteDialogLoading =
    deleteDialog.type === "bulk"
      ? bulkDeleting
      : !!(deleteDialog.id && deleteLoadingId === deleteDialog.id);
          ...
          <div className="px-6 py-4">
            <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {loading ? (
                      ...
                      `${totalItems} item${totalItems !== 1 ? "s" : ""} · Auto-deleted after 30 days${refreshing ? " · Refreshing..." : ""}`
                    )}
                  </p>
                </div>
              </div>
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => fetchBin(items.length === 0, currentPage, debouncedSearch)}
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  ...
                                  disabled={isActionLoading || deleteLoadingId === doc.id || bulkDeleting}
                                >
      ...
      >
        <AlertDialogContent className="max-w-md border-border/40">
          ...
          <AlertDialogFooter className="mt-4 gap-2 sm:gap-2">
            <AlertDialogCancel className="text-sm" disabled={isDeleteDialogLoading}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={(event) => {
