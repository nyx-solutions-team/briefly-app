...
const nodeTypeColors: Record<string, { bg: string; icon: string; label: string }> = {
  ...
  "pyserver.transform_document": { bg: "bg-zinc-500", icon: "text-zinc-500", label: "Transform" },
...
const nodeIcons: Record<string, React.ReactNode> = {
  ...
  "pyserver.transform_document": <FileText className="w-3 h-3" />,
...
export default function WorkflowsPage() {
  ...
  const isAdmin = bootstrapData?.permissions?.["org.manage_members"] === true;
  ...
  if (bootstrapData && !(isAdmin || isTeamLead)) {
    return (
      <AppLayout>
        <AccessDenied
          title="Workflows Restricted"
          message="Workflows are only available to organization owners, admins, and team leads."
        />
      </AppLayout>
    );
  }
  ...
  const reload = React.useCallback(async (force = false) => {
    ...
    try {
      const [cfg, tplRes, runRes] = await Promise.all([
        getWorkflowConfig({ force }),
        listWorkflowTemplates(false, true, { force }),
        listWorkflowRuns({ limit: 20 }, { force }),
      ]);
      setConfig(cfg);
      ...
      void (async () => {
        try {
          const nodeRes = await listWorkflowNodeDefinitions(false, { force });
