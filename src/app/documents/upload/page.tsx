...
function UploadContent() {
  ...
  const waitForIngestionJobReady = async (orgId: string, docId: string, initialJob?: IngestionJobRecord | null) => {
    ...
    while (true) {
      ...
      const hiddenBackoffMs = typeof document !== 'undefined' && document.visibilityState !== 'visible' ? 12000 : 0;
      await new Promise((resolve) => setTimeout(resolve, Math.max(delayMs, hiddenBackoffMs)));
      job = await fetchIngestionJobForDoc(orgId, docId);
    }
  };
  ...
  const waitForAnalysisJob = async (orgId: string, jobId: string): Promise<AnalyzeSuccessResponse> => {
    ...
    while (true) {
      ...
      const hiddenBackoffMs = typeof document !== 'undefined' && document.visibilityState !== 'visible' ? 12000 : 0;
      await new Promise((resolve) => setTimeout(resolve, Math.max(currentInterval, hiddenBackoffMs)));
    }
  };
  ...
  useEffect(() => {
    ...
    const pollEveryMs = itemsToWatch.length > 20 ? 5000 : itemsToWatch.length > 8 ? 4000 : 3000;
  ...
  }, [queue.map(q => `${q.docId}|${q.status}|${q.ingestionStatus}`).join(','), isPageVisible]); // Re-run when queue items / visibility change
