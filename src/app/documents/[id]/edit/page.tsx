...
import {
  ...
  type DocumentLinkEntry,
} from '@/components/document-links-content';
...
export default function EditDocumentPage() {
  ...
  const [linkDeleteTarget, setLinkDeleteTarget] = React.useState<{ id: string; title?: string } | null>(null);
  ...
  React.useEffect(() => {
    if (!addToChainOpen && !linkPickerOpen) return;
    if (hasLoadedAll) return;
    void loadAllDocuments();
  }, [addToChainOpen, hasLoadedAll, linkPickerOpen, loadAllDocuments]);
  ...
  const directLinks = React.useMemo<DocumentLinkEntry[]>(() => {
    const merged = new Map<string, DocumentLinkEntry>();

    const ingest = (items: RelationshipRecord[], direction?: 'incoming' | 'outgoing') => {
      for (const raw of items || []) {
        if (!raw?.id) continue;
        const id = String(raw.id);
        if (versionChainIds.has(id)) continue;
        ...
        merged.set(id, {
          id,
          title: raw.title || existing?.title || 'Untitled',
          type: raw.type ?? existing?.type ?? null,
          linkType: raw.linkType || existing?.linkType || 'related',
          linkedAt: raw.linkedAt || existing?.linkedAt || null,
          versionNumber: raw.versionNumber ?? existing?.versionNumber ?? null,
          isCurrentVersion: raw.isCurrentVersion ?? existing?.isCurrentVersion ?? false,
          directions: Array.from(directions),
        });
      }
    };

    ingest(Array.isArray(relationships.outgoing) ? (relationships.outgoing as RelationshipRecord[]) : [], 'outgoing');
    ingest(Array.isArray(relationships.incoming) ? (relationships.incoming as RelationshipRecord[]) : [], 'incoming');
    if (merged.size === 0) {
      ingest(Array.isArray(relationships.linked) ? (relationships.linked as RelationshipRecord[]) : []);
    }
  ...
  }, [relationships.incoming, relationships.linked, relationships.outgoing, versionChainIds]);

  const directLinkIds = React.useMemo(() => {
    return new Set(directLinks.map((item) => item.id));
  }, [directLinks]);

  const availableLinkDocuments = React.useMemo(() => {
    return documents.filter((item) => {
      if (!item?.id || item.id === doc?.id) return false;
      if (versionChainIds.has(item.id)) return false;
      if (directLinkIds.has(item.id)) return false;
      return (item as any)?.type !== 'folder';
    });
  }, [directLinkIds, doc?.id, documents, versionChainIds]);

  const handleCreateLink = React.useCallback(async (linkedId: string, overrideType?: string) => {
    if (!doc?.id) return;
    if (linkedId === doc.id) {
      toast({
        title: 'Pick a different document',
        description: "You can't link a document to itself.",
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const { orgId } = getApiContext();
      const result = await apiFetch<{ ok?: boolean; error?: string }>(`/orgs/${orgId}/documents/${doc.id}/link`, {
        method: 'POST',
        body: {
          linkedId,
          linkType: overrideType || linkType,
        },
      });
      if (result?.ok === false) {
        throw new Error(result.error || 'Could not link documents');
      }

      toast({
        title: 'Documents linked',
        description: `Added a ${overrideType || linkType} link.`,
      });
      ...
      toast({
        title: 'Link failed',
        description: error?.message || 'Could not link the selected document.',
        variant: 'destructive',
      });
    } finally {
      setLinkActionLoading(false);
    }
  }, [doc?.id, linkType, loadRelationships, toast]);
  ...
  return (
    <AppLayout>
        ...
        <main className={cn("flex-1", isMobile ? "bg-background px-4 pb-20 pt-24" : "px-6 py-6")}>
          {isDocLoading ? (
            ...
            <div className="flex flex-col gap-6">
              ...
              <Tabs defaultValue="overview" className="w-full">
                ...
                <TabsContent value="history" className="mt-0 focus-visible:outline-none focus:outline-none pb-12">
                  ...
                  <div className="mt-4">
                    <Section icon={LinkIcon} title="Linked Documents" className="bg-card/50 border-border/40 rounded-2xl p-6">
                      <DocumentLinksContent
                        links={directLinks}
                        loading={relLoading}
                        linkType={linkType}
                        ...
                        onRemoveLink={(item) => setLinkDeleteTarget({ id: item.id, title: item.title })}
                        compact
                        busy={linkActionLoading}
                        emptyMessage="Add related records when needed."
                      />
                    </Section>
                  </div>
                </TabsContent>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <Section icon={LinkIcon} title="Linked Documents">
                <DocumentLinksContent
                  links={directLinks}
                  loading={relLoading}
                  linkType={linkType}
      ...
      <AlertDialog open={!!linkDeleteTarget} onOpenChange={(open) => { if (!open) setLinkDeleteTarget(null); }}>
        <AlertDialogContent className={cn("max-w-md border-border/40", isMobile && "w-[90vw] rounded-3xl p-6")}>
          <AlertDialogHeader>
            <div className="flex items-start gap-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-amber-500/10">
              ...
              <div>
                ...
                <AlertDialogDescription className="mt-2 text-sm text-muted-foreground">
                  This will remove the direct link to “{linkDeleteTarget?.title || 'Untitled'}”. Both documents will still exist.
                </AlertDialogDescription>
              </div>
            </div>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-4 gap-2 sm:gap-2">
            <AlertDialogCancel className="text-sm rounded-xl" disabled={linkActionLoading}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                if (!linkDeleteTarget || !doc?.id) return;
                try {
                  setLinkActionLoading(true);
                  const { orgId } = getApiContext();
                  await apiFetch(`/orgs/${orgId}/documents/${doc.id}/link/${linkDeleteTarget.id}`, { method: 'DELETE' });
                  await loadRelationships();
                  setLinkDeleteTarget(null);
                  toast({
                    title: 'Link removed',
                    description: 'The direct relationship has been removed.',
                  });
                } catch (error) {
                  console.error('Failed to remove link:', error);
                  toast({
                    title: 'Remove link failed',
                    description: 'Could not remove this linked document.',
                    variant: 'destructive',
                  });
                } finally {
                  setLinkActionLoading(false);
      ...
      <DocumentPickerDialog
        ...
        documents={availableLinkDocuments}
        folders={folders}
        initialPath={form.folderPath.split('/').filter(Boolean)}
        selectedId={null}
        onSelect={(linkedId) => {
          void handleCreateLink(linkedId);
        }}
      />
