...
export default function VespaOCRMapPage() {
  ...
  useEffect(() => {
    if (isPageVisible && (v2Job?.status === 'running' || v2Job?.status === 'queued')) {
      ...
      }, v2Job?.status === 'running' ? 2000 : 4000);
  ...
  }, [v2Job?.status, fetchV2JobData, isPageVisible]);
  ...
  }, [isPageVisible, v2Job?.status, fetchV2JobData]);
