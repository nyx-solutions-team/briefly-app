...
export default function DocumentDetailPage() {
  ...
  const [linkDeleteTarget, setLinkDeleteTarget] = useState<{ id: string; title?: string } | null>(null);
  ...
  const handleCreateLink = useCallback(async (linkedId: string, overrideType?: string) => {
    if (!doc?.id) return;
    if (linkedId === doc.id) {
      toast({
  ...
  useEffect(() => {
    if (!linkPickerOpen || hasLoadedAll) return;
    void loadAllDocuments();
  }, [hasLoadedAll, linkPickerOpen, loadAllDocuments]);
    ...
    <AppLayout>
            ...
            <Tabs defaultValue="overview" className="flex-1 flex flex-col">
              ...
              <div className="p-4 overflow-y-auto flex-1">
                ...
                <TabsContent value="history" className="mt-0 focus-visible:outline-none focus:outline-none">
                  ...
                  <div className="mt-4">
                    <Section icon={Link2} title="Linked Documents" className="bg-card/50 border-border/40 rounded-2xl">
                      <DocumentLinksContent
                        links={directLinks}
                        loading={relLoading}
                        linkType={linkType}
          ...
          <main className={cn(
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 items-start">
              ...
              <div className="xl:col-span-3 space-y-6">
                ...
                <Section icon={Link2} title="Linked Documents" isMobile={isMobile}>
                  <DocumentLinksContent
                    links={directLinks}
                    loading={relLoading}
                    linkType={linkType}
      ...
      <AlertDialog open={!!linkDeleteTarget} onOpenChange={(open) => { if (!open) setLinkDeleteTarget(null); }}>
        <AlertDialogContent className="max-w-md border-border/40">
          ...
          <AlertDialogFooter className="mt-4 gap-2 sm:gap-2">
            <AlertDialogCancel className="text-sm" disabled={linkActionLoading}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
