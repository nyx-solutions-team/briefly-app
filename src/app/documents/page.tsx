...
function DocumentsPageContent() {
  ...
  const currentFolders = useMemo(() => listFolders(path), [listFolders, path]);
  const currentDocs = useMemo(() => getDocumentsInPath(path), [getDocumentsInPath, path]);
  ...
  // Helper to get the best display title for a document
  // Prioritizes: title → subject → cleaned filename
  const getDisplayTitle = useCallback((d: StoredDocument) => {
    ...
    // Fall back to name/filename but clean it up if it looks like a raw filename
    const fallback = d.name || (d as any).filename || 'Untitled';
    // If it starts with numbers (timestamp), try to extract a meaningful part
    const cleaned = fallback
      .replace(/^\d{10,}-/, '') // Remove leading timestamp like 1760091406738-
                    ...
                    >
                      <CardContent className="p-4">
                        ...
                        <div className="flex items-start justify-between gap-2 mb-3">
                          ...
                          {hasRoleAtLeast('teamLead') && deptName && (
            ...
            {effectiveView === 'list' ? (
                      ...
                      >
                        ...
                        <div className="flex-1 w-0 min-w-0 overflow-hidden">
                          <Link
                            ...
                            onDoubleClick={hasRoleAtLeast('member') && canUpdateDocuments ? (e) => { e.preventDefault(); startEdit(d); } : undefined}
                          >
                            <span
                            ...
                            {d.sender && (
                              <span className="text-xs text-muted-foreground truncate block" title={`From ${d.sender}`}>
                                From {d.sender}
                              </span>
                            )}
                          </Link>
                        ...
                        <div className="hidden lg:block w-28 overflow-hidden">
                          <span className="text-sm text-muted-foreground truncate block" title={d.sender || undefined}>{d.sender || '—'}</span>
                        </div>
                          ...
                          <CardContent className="p-3 sm:p-4">
                            <Link href={`/documents/${d.id}`} className="flex flex-col gap-2">
                              ...
                              {editingId === d.id ? (
                                ...
                                <div className="font-medium text-sm line-clamp-2" onDoubleClick={hasRoleAtLeast('member') && canUpdateDocuments ? (e) => { e.preventDefault(); startEdit(d); } : undefined}>{getDisplayTitle(d)}</div>
                              )}
                            </Link>
                          </CardContent>
                        </Card>
              ...
              <div className="space-y-3">
                {filteredDocs.map(d => {
                  const { icon: DocIcon, color: iconColor, bg: iconBg } = getFileTypeIcon((d as any).mimeType, d.filename || d.name);
                  return (
                    <Card key={d.id} className="bg-background/50 hover:bg-muted/30 hover:border-border/60 transition-all">
                      <CardContent className="p-3 sm:p-4 space-y-2.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <div className="font-medium text-sm line-clamp-2">{getDisplayTitle(d)}</div>
                            {d.aiPurpose && (
                              <p className="text-xs text-muted-foreground line-clamp-2">{d.aiPurpose}</p>
                            )}
                          </div>
                          {hasRoleAtLeast('systemAdmin') && renderDepartmentBadge((d as any).departmentId || (d as any).department_id || null)}
                        </div>
                        <div className="rounded-md border p-2.5 text-xs text-muted-foreground flex flex-wrap items-center gap-2">
                          <span>From <span className="text-foreground">{d.sender || '—'}</span> → To <span className="text-foreground">{d.receiver || '—'}</span></span>
                          <span className="ml-auto text-right">{formatNiceDate(d)}</span>
                        </div>
                        ...
                        <div className="text-right"><Link href={`/documents/${d.id}`} className="text-xs text-primary hover:underline">View</Link></div>
                      </CardContent>
                    </Card>
                  );
                })}
