...
import { Grid2X2, List, Grid3X3, Folder as FolderIcon, FileText, Trash2, ArrowLeft, X, FileImage, FileSpreadsheet, FileType, File, Home, ChevronRight, Share2, Copy, Check, Lock, Loader2 } from 'lucide-react';
...
function DocumentsPageContent() {
  ...
  const reloadFolderContents = useCallback(async (targetPathKey = pathKey) => {
  ...
  const removeIdsFromCurrentFolderContents = useCallback((ids: string[]) => {
    if (ids.length === 0) return;
    setFolderContents((prev) => {
      if (!prev || prev.pathKey !== pathKey) return prev;
      return {
        ...prev,
  ...
  const getDocumentsInSubtree = useCallback((targetPath: string[]) => {
    return documents.filter((doc) => {
      const folderPath = doc.folderPath || [];
      if (doc.type === 'folder' || folderPath.length < targetPath.length) return false;
      return targetPath.every((segment, index) => folderPath[index] === segment);
    });
  }, [documents]);
  const getFolderDeleteImpact = useCallback((targetPath: string[]) => {
    const subtreeDocumentCount = getDocumentsInSubtree(targetPath).length;
    const visibleItemCount = getFolderItemCount(targetPath);
    const effectiveCount = subtreeDocumentCount > 0 ? subtreeDocumentCount : visibleItemCount;

    return {
      subtreeDocumentCount,
      visibleItemCount,
      effectiveCount,
      hasAnyItems: effectiveCount > 0,
      countLabel: subtreeDocumentCount > 0 ? 'document(s)' : 'item(s)',
    };
  }, [getDocumentsInSubtree, getFolderItemCount]);
  ...
  const [deletionMode, setDeletionMode] = useState<'move_to_root' | 'move_to_recycle_bin'>('move_to_root');
    ...
    try {
      ...
      if (result.documentsHandled > 0) {
        if (deletionMode === 'move_to_root') {
          ...
          message += `. ${result.documentsHandled} document(s) moved to recycle bin.`;
        }
      }

      toast({ title: 'Success', description: message });
      await reloadFolderContents(path.join('/'));
  ...
  const bulkDelete = async () => {
    ...
    try {
      ...
      const result = await removeDocuments(ids);
      removeIdsFromCurrentFolderContents(ids);

      toast({
        title: 'Documents Deleted',
        description: `${result.deleted || ids.length} document(s) moved to recycle bin`,
      });
  ...
  const handleSingleDelete = async () => {
    ...
    try {
      ...
      removeIdsFromCurrentFolderContents([documentToDelete.id]);
  ...
  return (
    <AppLayout>
      <div className="min-h-screen flex flex-col">
              ...
              <div className="ml-auto flex flex-wrap items-center gap-2 w-full lg:w-auto">
                ...
                {hasPermission('documents.delete') && (
                  <Button
                    ...
                    disabled={bulkDeletePending || singleDeletePending}
                  >
                    {bulkDeletePending ? (
                      <>
        ...
        <Dialog open={!!folderToDelete} onOpenChange={(open) => {
          if (!isDeleting && !open) setFolderToDelete(null);
        }}>
          <DialogContent className="sm:max-w-md">
            ...
            {folderToDelete && (() => {
              const deleteImpact = getFolderDeleteImpact(folderToDelete);
              ...
              <div className="space-y-4">
                ...
                {deleteImpact.hasAnyItems && (
                  <div className="space-y-3">
                    <p className="text-sm font-medium">
                      This folder and its subfolders contain {deleteImpact.effectiveCount} {deleteImpact.countLabel}. What would you like to do?
                    </p>
                    {deleteImpact.subtreeDocumentCount === 0 && (
                      <p className="text-xs text-muted-foreground">
                        Some contents may not be present in the current in-memory document list, but this folder is not empty.
                      </p>
                    )}

                    <RadioGroup
                      value={deletionMode}
                      onValueChange={(value: 'move_to_root' | 'move_to_recycle_bin') => setDeletionMode(value)}
                      disabled={isDeleting}
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="move_to_recycle_bin" id="delete" />
                        <label htmlFor="delete" className="text-sm">
                          <span className="font-medium">Move all documents to recycle bin</span>
                          <br />
                          <span className="text-muted-foreground">Documents will be removed from the library now and permanently deleted after 7 days</span>
                        </label>
                      </div>
                    </RadioGroup>
                  </div>
            ...
            <DialogFooter>
              ...
              <Button
                variant={deletionMode === 'move_to_recycle_bin' ? 'secondary' : 'default'}
                onClick={handleFolderDeletion}
                disabled={isDeleting}
              >
                {isDeleting ? 'Deleting...' : (
                  deletionMode === 'move_to_recycle_bin' ? 'Move to Recycle Bin' : 'Delete Folder'
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={confirmBulkDeleteOpen} onOpenChange={(open) => {
          if (!bulkDeletePending) setConfirmBulkDeleteOpen(open);
        }}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Move Documents to Recycle Bin</DialogTitle>
            </DialogHeader>

            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Are you sure you want to move <span className="font-medium">{selectedIds.size} document(s)</span> to the recycle bin?
                You can restore them for 7 days before they are permanently deleted.
              </p>

              <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-3">
                <p className="text-sm font-medium text-amber-700 dark:text-amber-300">
                  These documents will be removed from the library immediately and scheduled for permanent deletion in 7 days.
                </p>
              </div>
            </div>
        ...
        <Dialog open={confirmDeleteOpen} onOpenChange={(open) => {
          if (!singleDeletePending) setConfirmDeleteOpen(open);
        }}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Move Document to Recycle Bin</DialogTitle>
            </DialogHeader>

            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Are you sure you want to move <span className="font-medium">"{documentToDelete?.title || documentToDelete?.name}"</span> to the recycle bin?
                You can restore it for 7 days before it is permanently deleted.
              </p>

              <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-3">
                <p className="text-sm font-medium text-amber-700 dark:text-amber-300">
                  This document will be removed from the library immediately and scheduled for permanent deletion in 7 days.
                </p>
              </div>
            </div>
