...
export function WorkflowExecuteStudio({
  ...
  const templateRequirements = React.useMemo<InputRequirement[]>(() => {
    ...
    for (let index = 0; index < nodes.length; index += 1) {
      ...
      const hasStaticSourceText = typeof config?.source_text === "string" && String(config.source_text).trim().length > 0;
      const hasStaticSourceData = Boolean(config?.source_data && typeof config.source_data === "object" && Object.keys(config.source_data || {}).length > 0);
      const hasSourceTextBinding = typeof inputBindings?.source_text === "string" && String(inputBindings.source_text).trim().length > 0;
      const hasSourceDataBinding = typeof inputBindings?.source_data === "string" && String(inputBindings.source_data).trim().length > 0;
      ...
      if (
        nodeType === "pyserver.transform_document"
        && !hasStaticDocs
        && !hasDocsFromInput
        && !hasDocsFromSteps
        && !hasStaticSourceData
        && !hasSourceDataBinding
        && !hasStaticSourceText
        && !hasSourceTextBinding
        && !hasTextBinding
        && !hasContentFromInput
        && !hasContentFromSteps
        && !hasImplicitPreviousStepOutput
      ) {
        addRequirement("source_data", nodeId, nodeType);
      }
    }
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      if (!isPageVisible) {
        currentTimer = window.setTimeout(() => { void tick(); }, 2500);
  ...
  }, [liveRunId, loadRun, isPageVisible]);

  React.useEffect(() => {
    if (!isPageVisible) return;
    if (!liveRunId) return;
    void loadRun(liveRunId);
  }, [isPageVisible, liveRunId, loadRun]);
