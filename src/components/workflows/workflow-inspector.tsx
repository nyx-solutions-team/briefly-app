...
function suggestedPyserverTransformSourcePath(previousNodeId: string, previousNodeType: string): string {
  const nodeId = String(previousNodeId || "").trim();
  if (!nodeId) return "";
  const nodeType = normalizeNodeType(previousNodeType || "");
  if (nodeType === "dms.read_document") return `$.steps.${nodeId}.output.doc.text`;
  if (nodeType === "ai.extract") return `$.steps.${nodeId}.output.response_json`;
  if (nodeType === "ai.prompt" || nodeType === "ai.summary") return `$.steps.${nodeId}.output.response_text`;
  if (nodeType === "dms.create_document") return `$.steps.${nodeId}.output.document_json`;
  return `$.steps.${nodeId}.output`;
}
...
const FALLBACK_NODE_OPTIONS: Array<{ key: string; label: string }> = [
  ...
  { key: "pyserver.transform_document", label: "Pyserver Transform" },
...
export function WorkflowInspector(props: Props) {
  ...
  const defaultTransformSourcePath = React.useMemo(
    () => suggestedPyserverTransformSourcePath(String(previousNodeId || ""), String(previousNodeType || "")),
    [previousNodeId, previousNodeType]
  );
  ...
  const isPyserverTransformNode = nodeType === "pyserver.transform_document";
  ...
  React.useEffect(() => {
    if (!isPyserverTransformNode) return;
    const hasSourceDataBinding = String(selectedBindings?.source_data || "").trim().length > 0;
    const hasSourceTextBinding = String(selectedBindings?.source_text || "").trim().length > 0;
    if (hasSourceDataBinding || hasSourceTextBinding) return;
    if (!defaultTransformSourcePath) return;
    const preferredKey = defaultTransformSourcePath.includes(".response_json")
      ? "source_data"
      : "source_text";
    onPatchSelectedNode({
      input_bindings: {
        ...selectedBindings,
        [preferredKey]: defaultTransformSourcePath,
      },
    });
  }, [defaultTransformSourcePath, isPyserverTransformNode, onPatchSelectedNode, selectedBindings]);
  ...
  const bindingFields = React.useMemo(() => {
    ...
    if (isPyserverTransformNode) {
      return [
        { key: "doc_id", label: "Source Document (single)", placeholder: "$.input.doc_id", runInputPath: "$.input.doc_id" },
        { key: "doc_ids", label: "Source Documents", placeholder: "$.input.doc_ids", runInputPath: "$.input.doc_ids" },
        { key: "source_data", label: "Source JSON", placeholder: "$.steps.extract.output.response_json", runInputPath: "$.input.source_data" },
        { key: "source_text", label: "Source Text", placeholder: "$.steps.read_doc.output.doc.text", runInputPath: "$.input.source_text" },
      ];
    }
  ...
  const isDocInputNode = isAiPromptNode || isAiExtractNode || isAiClassifyNode || isDmsReadNode || isDmsMoveNode || isDmsSetMetadataNode || isPyserverTransformNode;
          ...
          {!hasSelectedNode ? (
            ...
            <>
              <div className="p-3 space-y-3 flex-1 overflow-auto">
                ...
                {isPyserverTransformNode ? (
                  <div className="space-y-2 rounded border border-border/40 p-2 bg-background/40">
                    <div className="text-sm font-medium">Pyserver Transform Settings</div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">Source Type</div>
                        <Input
                          value={String(selectedConfig.source_type || "quotation")}
                          onChange={(e) => patchConfigField("source_type", e.target.value)}
                          className="h-9 text-sm"
                          placeholder="quotation"
                        />
                      </div>
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">Target Template</div>
                        <Input
                          value={String(selectedConfig.target_template || "purchase_order")}
                          onChange={(e) => patchConfigField("target_template", e.target.value)}
                          className="h-9 text-sm"
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">Output Title (PDF)</div>
                        <Input
                          value={String(selectedConfig.title || "")}
                          onChange={(e) => patchConfigField("title", e.target.value)}
                          className="h-9 text-sm"
                      ...
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">Output Folder</div>
                        <Input
                          value={dmsCreateFolderPath}
                          onChange={(e) => patchConfigField("folder_path", toPathArray(e.target.value))}
                          className="h-9 text-sm font-mono"
                          placeholder="Workflows/Generated"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">PDF Filename (optional)</div>
                        <Input
                          value={String(selectedConfig.pdf_filename || "")}
                          onChange={(e) => patchConfigField("pdf_filename", e.target.value)}
                          className="h-9 text-sm"
                          placeholder="PO-123.pdf"
                        />
                      </div>
                      <div>
                        <div className="text-sm text-muted-foreground mb-1">JSON Filename (optional)</div>
                        <Input
                          value={String(selectedConfig.json_filename || "")}
                          onChange={(e) => patchConfigField("json_filename", e.target.value)}
                          className="h-9 text-sm"
                          placeholder="PO-123.json"
                        />
                      </div>
                    </div>
                  </div>
                ...
                {bindingFields.length > 0 ? (
                  <div className="space-y-2 rounded border border-border/40 p-2 bg-background/40">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {isPyserverTransformNode && defaultTransformSourcePath ? (
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-8 px-2 text-sm"
                          onClick={() => patchBindingField(
                            defaultTransformSourcePath.includes(".response_json") ? "source_data" : "source_text",
                            defaultTransformSourcePath
                          )}
                        >
                          Use Previous Step Output
                        </Button>
