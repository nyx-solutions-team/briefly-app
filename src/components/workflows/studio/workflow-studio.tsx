...
export function runningLine(nodeTypeRaw: string, subjectCount: number): string {
  ...
  if (nodeType === "pyserver.transform_document") return "Transforming source document via pyserver and generating outputs.";
...
const CATALOG: NodeCatalogItem[] = [
  ...
  },
  {
    ...
    modes: [
      ...
      { value: "transform", label: "Transform (Pyserver)", nodeKey: "pyserver.transform_document" },
...
function defaultConfigFor(type: string, mode: string): Record<string, any> {
  ...
  if (type === "document" && mode === "transform") {
    return {
      source_type: "quotation",
      target_template: "purchase_order",
      folder_path: "/",
...
function explicitInputTargetsForNode(node: WorkflowNode): string[] {
  ...
  if (node.type === "document" && mode === "transform") return ["doc_ids", "source_data", "source_text"];
...
function preferredDocIdsPathFromSource(sourceNode: WorkflowNode): string {
  ...
  if (sourceNode.type === "document" && (mode === "create" || mode === "transform")) return buildStepOutputPath(sourceNode.id, "generated_doc_id");
...
function autoBindingRulesForTarget(targetNode: WorkflowNode): AutoBindingRule[] {
  ...
  if (targetNode.type === "document" && mode === "transform") {
    return [
      {
        targetKey: "doc_ids",
        bindingKeys: ["doc_ids", "doc_id"],
      ...
      {
        targetKey: "source_data",
        bindingKeys: ["source_data", "records"],
        configKeys: ["source_data", "records"],
        runInputFallback: "$.input.source_data",
        fromSource: preferredRecordsPathFromSource,
      },
      {
        targetKey: "source_text",
        bindingKeys: ["source_text", "text", "content"],
        configKeys: ["source_text", "text", "content"],
        runInputFallback: "$.input.source_text",
        fromSource: preferredTextPathFromSource,
      },
    ];
  }
...
function missingInputsForNode(
  ...
  if (node.type === "document" && mode === "transform") {
    requireAny(
      "Source input (document IDs, source JSON, or source text)",
      ["doc_ids", "doc_id", "source_data", "source_text", "text"],
      ["doc_ids", "doc_id", "source_data", "source_text", "text"],
      { satisfyWithIncoming: true }
    );
    return missing;
  }
...
function targetNeeds(node: WorkflowNode): Array<"docs" | "text_or_docs" | "records" | "content"> {
  ...
  if (node.type === "document" && mode === "create") return ["content"];
  if (node.type === "document" && mode === "transform") return ["text_or_docs"];
...
function suggestedNextNodes(sourceNode: WorkflowNode): string[] {
  ...
  if (sourceNode.type === "document" && mode === "transform") return ["Checks -> Validate", "Human -> Review", "Output -> Export CSV"];
...
export function WorkflowStudio({ initialTemplateId = null, initialRunId = null, onRunWorkflow, onOpenHistory, onBackToHome }: WorkflowStudioProps = {}) {
  ...
  React.useEffect(() => {
    if (!activeRunId || viewMode !== "run" || !isPageVisible) return;
  ...
  }, [activeRunId, viewMode, isPageVisible]);
      ...
      if (selectedMode === "transform") {
        return (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em] px-1">Source Type</label>
                <Input
                  className="h-9 text-xs border-border/40 bg-transparent focus-visible:bg-muted/5 transition-colors"
                  value={readString(selectedConfig.source_type, "quotation")}
                  onChange={(e) => updateNodeConfig("source_type", e.target.value)}
                  placeholder="quotation"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em] px-1">Target Template</label>
                <Input
                  className="h-9 text-xs border-border/40 bg-transparent focus-visible:bg-muted/5 transition-colors"
                  value={readString(selectedConfig.target_template, "purchase_order")}
                  onChange={(e) => updateNodeConfig("target_template", e.target.value)}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em] px-1">Source Document(s)</label>
              <div className="flex items-center gap-2">
                <Input
                  className="h-9 text-xs border-border/40 bg-muted/5"
                  value={readString(readStringArray(selectedConfig.doc_ids).join(", "), "")}
                  placeholder="Optional: select quotation documents"
                  readOnly
                />
                <Button type="button" variant="outline" size="sm" className="h-9 text-[11px] font-bold border-border/40 hover:bg-muted/10 shrink-0" onClick={() => openConfigDocPicker("doc_ids", true, 20)}>
                  Pick
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em] px-1">Output Title</label>
                <Input
                  className="h-9 text-xs border-border/40 bg-transparent focus-visible:bg-muted/5 transition-colors"
                  value={readString(selectedConfig.title, "Purchase Order")}
                  onChange={(e) => updateNodeConfig("title", e.target.value)}
                  placeholder="Purchase Order"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em] px-1">Output Folder</label>
                <div className="flex items-center gap-2">
                  <Input
                    className="h-9 text-xs border-border/40 bg-muted/5"
                    value={readString(selectedConfig.folder_path, "")}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em] px-1">PDF Filename (Optional)</label>
                <Input
                  className="h-9 text-xs border-border/40 bg-transparent focus-visible:bg-muted/5 transition-colors font-mono"
                  value={readString(selectedConfig.pdf_filename, "")}
                  onChange={(e) => updateNodeConfig("pdf_filename", e.target.value)}
                  placeholder="PO-123.pdf"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em] px-1">JSON Filename (Optional)</label>
                <Input
                  className="h-9 text-xs border-border/40 bg-transparent focus-visible:bg-muted/5 transition-colors font-mono"
                  value={readString(selectedConfig.json_filename, "")}
                  onChange={(e) => updateNodeConfig("json_filename", e.target.value)}
                      ...
                      <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        {!step?.output ? (
                          ...
                          <div className="space-y-4">
                            ...
                            {/* Document created */}
                            {(nodeTypeRaw.includes("create_document") || nodeTypeRaw.includes("transform_document") || generatedDocId) && (
