...
export default function AppLayout({ children, collapseSidebar = false, flush = false }: { children: React.ReactNode; collapseSidebar?: boolean; flush?: boolean }) {
  ...
  // Treat `collapseSidebar` as an auto-collapse trigger, not a hard lock.
  // This lets pages request "collapse now" while still allowing users to reopen manually.
  const [sidebarOpen, setSidebarOpen] = React.useState(!collapseSidebar);
  const lastCollapseSidebarRef = React.useRef(collapseSidebar);

  React.useEffect(() => {
    const prev = lastCollapseSidebarRef.current;
    if (collapseSidebar && !prev) {
      setSidebarOpen(false);
    }
    lastCollapseSidebarRef.current = collapseSidebar;
  }, [collapseSidebar]);
      ...
      <Sidebar variant="inset" collapsible="icon" className="border-r-0">
        ...
        <SidebarHeader className="p-0">
          <div className="flex items-center gap-2 px-3 py-3 border-b border-sidebar-border/30 group-data-[collapsible=icon]:flex-col group-data-[collapsible=icon]:px-0 group-data-[collapsible=icon]:py-4">
            <div className="hidden h-7 w-7 items-center justify-center rounded-md bg-primary/10 group-data-[collapsible=icon]:mb-1 group-data-[collapsible=icon]:flex">
              <img src="/favicon.ico" alt="Briefly" className="h-4 w-4" />
            </div>
            <Link
              href="/dashboard"
              className="text-[1.35rem] font-semibold tracking-[-0.03em] text-sidebar-foreground hover:text-sidebar-foreground/80 transition-colors group-data-[collapsible=icon]:hidden"
            >
              Briefly
            </Link>
