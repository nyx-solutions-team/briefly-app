...
type PickerDocument = StoredDocument & {
  filename?: string;
  folder_path?: string[];
  subject?: string;
  sender?: string;
...
export type DocumentPickerDialogProps = {
  open: boolean;
...
export function DocumentPickerDialog({
  ...
  const allDocs = useMemo(() => {
    return documents
      ...
      .map((doc) => {
        const currentDoc = doc as PickerDocument;
        const displayTitle = String((currentDoc.title || currentDoc.name || currentDoc.filename || "Untitled") ?? "Untitled");
        const folderPath = (currentDoc.folderPath || currentDoc.folder_path || []).filter(Boolean);
    ...
    return folders
      ...
      .map((p) => {
        ...
        const hasChildren = folders.some(
          (other) => other.length > p.length && other.slice(0, p.length).join("/") === pathStr
        );
        return {
  ...
  const docsInViewingPath = useMemo(() => {
    return allDocs
      .filter((d) => d.folderPath.join("/") === viewingPath.join("/"))
      .sort((a, b) => a.displayTitle.localeCompare(b.displayTitle));
  }, [allDocs, viewingPath]);
  ...
  const searchResults = useMemo(() => {
    ...
    return allDocs
      .map((item) => {
        const doc = item.doc as PickerDocument;
  ...
  const visibleItems: Item[] = useMemo(() => {
    if (query.trim()) return searchResults.slice(0, limit);
    if (mode === "all") return allDocs.slice(0, limit);
    return [...folderChildren, ...docsInViewingPath];
  }, [allDocs, docsInViewingPath, folderChildren, limit, mode, query, searchResults]);
  ...
  const handleChooseDoc = useCallback((docId: string) => {
    onSelect(docId);
    onOpenChange(false);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter") {
        ...
        if (item.kind === "doc") {
          handleChooseDoc(item.doc.id);
        }
      }
    };
    document.addEventListener("keydown", onKeyDown);
    return () => document.removeEventListener("keydown", onKeyDown);
  }, [enterFolder, handleChooseDoc, mode, navigateToPath, open, query, selectedIndex, viewingPath, visibleItems]);
          ...
          {visibleItems.length === 0 ? (
            ...
            <ul className="p-1.5 space-y-0.5">
              {visibleItems.map((item, index) => {
                ...
                const doc = item.doc;
                const currentDoc = doc as PickerDocument;
                const ver = currentDoc.versionNumber || currentDoc.version || 1;
                ...
                return (
                  ...
                  >
                    ...
                    {isChosen ? <Check className="h-4 w-4 text-muted-foreground shrink-0" /> : null}
                  </li>
                );
              })}
            </ul>
