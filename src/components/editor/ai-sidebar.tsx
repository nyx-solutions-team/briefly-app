...
import { generateBlockDiff, hasChanges } from "@/components/editor/diff/diff-utils";
...
type ManualContextSource = "editor" | "documents";

type ManualContextFile = {
  id: string;
  source: ManualContextSource;
  filename: string;
  title?: string;
  folderPath: string[];
  type?: string;
...
function paragraphNodeFromTextValue(text: string): JSONContent {
  const normalized = normalizeNewlines(String(text || "")).trim();
...
function inferNotionCalloutTone(emoji: string, content: string): "tip" | "note" | "warning" | "success" {
  const probe = `${String(emoji || "")} ${String(content || "")}`.toLowerCase();
  if (/(warning|risk|alert|critical|danger|⚠)/i.test(probe)) return "warning";
  if (/(info|note|ℹ)/i.test(probe)) return "note";
  if (/(success|done|complete|approved|✅|✔)/i.test(probe)) return "success";
  return "tip";
}

function notionBlocksToTiptapContent(rawBlocks: any, titleFallback?: string): JSONContent[] | null {
  ...
  for (const block of blocks) {
    ...
    const type = String((block as any).type || "").trim().toLowerCase();
    if (!type) continue;

    if (type === "heading1" || type === "heading2" || type === "heading3") {
      const text = String((block as any).content || "").trim();
      if (!text) continue;
    ...
    if (type === "paragraph") {
      const text = String((block as any).content || "").trim();
      if (!text) continue;
      nodes.push(paragraphNodeFromTextValue(text));
      continue;
    }

    if (type === "bullets") {
      const items = toTextList((block as any).items);
      if (items.length === 0) continue;
      nodes.push({
        type: "bulletList",
        content: items.map((text) => ({
          type: "listItem",
          content: [paragraphNodeFromTextValue(text)],
        })),
      });
      continue;
    }
    ...
    if (type === "checklist") {
      const rawItems = Array.isArray((block as any).items) ? (block as any).items : [];
      ...
      nodes.push({
        type: "taskList",
        content: items.map((entry: any) => ({
          type: "taskItem",
          attrs: { checked: Boolean(entry.checked) },
          content: [paragraphNodeFromTextValue(String(entry.text || ""))],
        })),
      });
      continue;
    }
    ...
    if (type === "callout") {
      const content = String((block as any).content || "").trim();
      if (!content) continue;
      const emoji = String((block as any).emoji || "").trim();
      nodes.push({
        type: "callout",
        attrs: {
          tone: inferNotionCalloutTone(emoji, content),
          ...(emoji ? { emoji } : {}),
        },
        content: [paragraphNodeFromTextValue(content)],
      });
      continue;
    }
    ...
    if (type === "table") {
      const rawHeaders = Array.isArray((block as any).headers) ? (block as any).headers : [];
      const rawRows = Array.isArray((block as any).rows) ? (block as any).rows : [];

      const headers: string[] = rawHeaders.map((h: any) => String(h ?? "").trim());
      ...
      const normalizedHeaders = Array.from({ length: width }, (_, idx) => {
        const value = headers[idx];
        return value && value.length > 0 ? value : `Column ${idx + 1}`;
      });
      const normalizedRows: string[][] = rows.map((row: string[]) => {
        if (row.length < width) return row.concat(Array.from({ length: width - row.length }, () => ""));
        return row.slice(0, width);
      });

      const makeCellParagraph = (value: string): JSONContent => {
      ...
      nodes.push({
        ...
        content: [
          {
            type: "tableRow",
            content: normalizedHeaders.map((value: string) => ({
              type: "tableHeader",
              content: [makeCellParagraph(value)],
            })),
          },
          ...normalizedRows.map((row: string[]) => ({
            type: "tableRow",
            content: row.map((value: string) => ({
              type: "tableCell",
              content: [makeCellParagraph(value)],
            })),
          })),
  ...
  const trimmedTitle = String(titleFallback || "").trim();
  if (trimmedTitle && !nodes.some((node) => node.type === "heading" && Number((node as any).attrs?.level || 0) === 1)) {
    nodes.unshift({
      type: "heading",
      attrs: { level: 1 },
...
function manualContextFileKey(file: Pick<ManualContextFile, "id" | "source">) {
  return `${file.source}:${file.id}`;
}

function manualContextSourceLabel(source: ManualContextSource) {
  return source === "editor" ? "Editor" : "Uploaded";
}
...
function assistantTextToTiptapContent(text: string) {
  ...
  const looksImplicitHeading = (line: string) => {
    const raw = String(line || "").trim();
    if (!raw) return false;
    if (isBulletLine(raw) || isOrderedLine(raw) || isTaskLine(raw) || isQuoteLine(raw) || isDividerLine(raw)) return false;
    if (parseHeading(raw)) return true;
  ...
  for (let i = 0; i < rawLines.length; i += 1) {
    ...
    const prevNonEmpty = [...rebuiltLines].reverse().find((line) => String(line || "").trim().length > 0) || "";
    let nextNonEmpty = "";
    for (let j = i + 1; j < rawLines.length; j += 1) {
      const candidate = String(rawLines[j] || "").trim();
      if (candidate) {
    ...
    const prevIsList = isBulletLine(prevNonEmpty) || isOrderedLine(prevNonEmpty) || isTaskLine(prevNonEmpty);
    const nextIsList = isBulletLine(nextNonEmpty) || isOrderedLine(nextNonEmpty) || isTaskLine(nextNonEmpty);
    const shouldStartNewBlock =
      trimmedCurrent &&
      rebuiltLines.length > 0 &&
      prevIsList &&
      nextIsList &&
      looksImplicitHeading(trimmedCurrent);

    if (shouldStartNewBlock && rebuiltLines[rebuiltLines.length - 1] !== "") {
      rebuiltLines.push("");
    }
    rebuiltLines.push(current);
  }

  const blocks = rebuiltLines.join("\n").split(/\n{2,}/g);
  const nodes: any[] = [];
...
function normalizePickedDoc(doc: StoredDocument, source: ManualContextSource): ManualContextFile {
...
export function AiSidebar({ editor, className, docId, versionId, sessionId }: Props) {
  ...
  const [pickerSource, setPickerSource] = React.useState<ManualContextSource>("editor");
  ...
  const openManualPicker = React.useCallback((source: ManualContextSource) => {
    setPickerSource(source);
    setMentionOpen(false);
    setPickerOpen(true);
  }, []);

  const currentPickerSelectedIds = React.useMemo(
    () => manualFiles.filter((file) => file.source === pickerSource).map((file) => file.id),
    [manualFiles, pickerSource]
  );
  const manualFileCountOutsidePickerSource = React.useMemo(
    () => manualFiles.filter((file) => file.source !== pickerSource).length,
    [manualFiles, pickerSource]
  );
  const pickerMaxDocs = Math.max(1, 8 - manualFileCountOutsidePickerSource);
  ...
  const refreshAutoContext = React.useCallback(() => {
    ...
    setAttached((prev) => {
      ...
      if (candidate.kind === "document" && prev && prev.kind !== "document") {
        return prev;
      }
    ...
    if (diffManager) {
      try {
        ...
        if (range.isSingleParagraphOrHeading) {
          ...
          if (!hasChanges(diffSegments)) {
            setError("Suggestion did not produce a visible change.");
            return;
          }
  ...
  const applyReplaceDocument = React.useCallback((messageId: string, meta: Extract<AssistantMeta, { kind: "replace_document" }>) => {
    ...
    const content = (Array.isArray(meta.docContent) && meta.docContent.length > 0)
      ? meta.docContent
      : assistantTextToTiptapContent(meta.docText);
    if (!content) {
      setError("Nothing to apply.");
      return;
    }
  ...
  const send = React.useCallback(async () => {
    ...
    let routedPayload: Awaited<ReturnType<typeof routeAndBuildPayload>> | null = null;
    ...
    if (tableJsonIntent) {
      ...
      const payload = routedPayload = await routeAndBuildPayload(userPrompt, editor, docId, versionId);
    ...
    try {
      const routed = routedPayload || await routeAndBuildPayload(userPrompt, editor, docId, versionId);
      const context = routed.content;
      ...
      const data = await apiFetch<any>(`/orgs/${orgId}/editor/ai/chat`, {
        ...
        body: {
          ...
          docId,
          context,
          docOverview: routed.docOverview,
          ...
          ...(manualFiles.length > 0
            ? {
              manualContextDocs: manualFiles.map((file) => ({
                id: file.id,
                source: file.source,
      ...
      const actionType = String(data?.action_type || data?.actionType || "").trim().toLowerCase();
      const actionPayload = (data && typeof data.payload === "object" && data.payload) ? data.payload : {};
      const serverMessage = String(data?.assistant_message || data?.message || "").trim();

      let meta: AssistantMeta;
      let messageContent = "";

      if (actionType === "replace_document") {
        const title = String(actionPayload?.title || data?.title || "").trim();
        const blocks = Array.isArray(actionPayload?.blocks)
          ? actionPayload.blocks
          : (Array.isArray(data?.blocks) ? data.blocks : []);

        const notionContent = notionBlocksToTiptapContent(blocks, title || "Untitled");
        if (!notionContent || notionContent.length === 0) {
          throw new Error("Assistant returned an empty or invalid document.");
        }
        ...
        meta = {
          ...
          status: "pending",
          docText: "",
          docContent: notionContent,
          title: title || undefined,
        };

        messageContent = serverMessage || (title ? `Document ready: ${title}` : "Document rewrite ready.");
      } else if (actionType === "table_json") {
        let tableInfo: TableRuntimeInfo | null = null;
        ...
        if (!tableInfo && editor.isActive("table")) {
          ...
          for (let depth = $from.depth; depth > 0; depth -= 1) {
            if ($from.node(depth)?.type?.name === "table") {
              tableInfo = getTableRuntimeInfoByTablePos(editor, $from.before(depth));
              break;
            }
          }
        }
        ...
        if (!tableInfo) {
          const query = normalizeSectionKey(String(actionPayload?.table_heading_query || actionPayload?.table_query || ""));
          const tables = collectTablesWithHeadings(editor);

          if (tables.length === 1) {
            tableInfo = getTableRuntimeInfoByTablePos(editor, tables[0].pos);
          } else if (query) {
            let bestPos: number | null = null;
            let bestScore = -1;
            for (const table of tables) {
              const headingKey = normalizeSectionKey(String(table.heading || ""));
              if (!headingKey) continue;
              if (!headingKey.includes(query)) continue;
              const score = 1000 - headingKey.length;
              if (score > bestScore) {
                bestScore = score;
                bestPos = table.pos;
              }
            ...
            if (bestPos != null) {
              tableInfo = getTableRuntimeInfoByTablePos(editor, bestPos);
            }
          }
        ...
        if (!tableInfo) {
          throw new Error("No table found to apply AI table instruction.");
        }

        const payloadInstruction = actionPayload?.instruction;
        const parsedInstruction = payloadInstruction && typeof payloadInstruction === "object"
          ? { value: payloadInstruction, error: undefined as string | undefined }
          : tryParseJsonObjectFromText(String(actionPayload?.instruction_raw || actionPayload?.instructionRaw || ""));

        const expectedAddRowCount = Number.isFinite(Number(actionPayload?.expected_add_row_count))
          ? Math.max(1, Math.trunc(Number(actionPayload.expected_add_row_count)))
          : null;

        const validated = parsedInstruction.value
          ...
          : { instruction: null, error: parsedInstruction.error || "Invalid JSON" };

        const instruction = validated.instruction;
        const instructionError = validated.error
          ? `${validated.error}. JSON required for table edits.`
          : undefined;
        const desc = instruction ? describeTableInstruction(instruction) : null;

        const tableLabel = String(actionPayload?.table_label || actionPayload?.tableLabel || "").trim()
          || (effectiveAttachment?.kind === "cells"
            ? effectiveAttachment.label
            : tableInfo.heading
              ? `Table: ${tableInfo.heading}`
              : `Table (${tableInfo.columnCount} cols)`);
        ...
        meta = {
          ...
          columns: tableInfo.columns,
          rowKeys: tableInfo.rowKeys,
          instructionRaw: instruction ? JSON.stringify(instruction) : String(actionPayload?.instruction_raw || ""),
          instruction: instruction || undefined,
          instructionError,
          preview: desc?.preview,
        };
        messageContent = desc?.title || serverMessage || "Table instruction ready.";
      } else if (actionType === "selection") {
        const text = String(actionPayload?.text || actionPayload?.content || "").trim();
        if (!text) throw new Error("Assistant returned an incomplete response. Please retry.");
        ...
        } else if (effectiveAttachment?.kind === "selection") {
          selectionFrom = effectiveAttachment.selectionFrom;
          selectionTo = effectiveAttachment.selectionTo;
          sourceText = effectiveAttachment.text;
        } else if (Number.isFinite(Number(actionPayload?.selection_from)) && Number.isFinite(Number(actionPayload?.selection_to))) {
          selectionFrom = Math.max(0, Number(actionPayload.selection_from));
          selectionTo = Math.max(selectionFrom, Number(actionPayload.selection_to));
        } else if (editor.state.selection.from !== editor.state.selection.to) {
          selectionFrom = editor.state.selection.from;
          selectionTo = editor.state.selection.to;
          sourceText = getSelectionTextForPrompt(editor, selectionFrom, selectionTo);
        }

        if (selectionFrom != null && selectionTo != null && selectionTo > selectionFrom) {
          const validationError = validateFreeformAssistantOutput(text);
          meta = {
            kind: "selection",
            status: "pending",
            selectionFrom,
      ...
      } else if (actionType === "move_section") {
        const sourceQuery = String(actionPayload?.source_query || actionPayload?.source || "").trim();
        const targetQuery = String(actionPayload?.target_query || actionPayload?.target || "").trim();
        const direction = String(actionPayload?.direction || "below").toLowerCase() === "above" ? "above" : "below";

        if (!sourceQuery || !targetQuery) {
          throw new Error("Move section response missing source/target.");
        }

        const sourceIdx = getHeadingIndexByQuery(editor, sourceQuery);
        const targetIdx = getHeadingIndexByQuery(editor, targetQuery);
        if (sourceIdx == null || targetIdx == null) {
          throw new Error("Could not find one of the sections by heading.");
        }

        const sourceRange = getSectionRangeByHeadingIndex(editor, sourceIdx);
        const targetRange = getSectionRangeByHeadingIndex(editor, targetIdx);
        if (!sourceRange || !targetRange) {
          throw new Error("Could not resolve section ranges.");
        }

        const dest = direction === "above" ? targetRange.start : targetRange.end;
        meta = {
          kind: "move_section",
          status: "pending",
          sourceTitle: sourceRange.title,
        ...
        messageContent = serverMessage || `Move "${sourceRange.title}" ${direction} "${targetRange.title}".`;
      } else if (actionType === "divider") {
        const mode = String(actionPayload?.mode || "cursor").toLowerCase();
        const headingQuery = String(actionPayload?.heading_query || actionPayload?.target_query || "").trim();

        let insertPos = cursorPos;
        let label: string | undefined;

        if ((mode === "before_heading" || headingQuery) && headingQuery) {
          const heading = findFirstHeadingContaining(editor, headingQuery);
          if (heading) {
            insertPos = heading.pos;
            label = `Before "${heading.text}"`;
          }
        } else if (mode === "before_current_heading") {
          const nearestHeading = findLastHeadingBefore(editor, cursorPos);
          if (nearestHeading) {
            insertPos = nearestHeading.pos;
            label = `Before "${nearestHeading.text}"`;
          }
        ...
        messageContent = serverMessage || (label ? `Divider - ${label}` : "Divider");
      } else {
        const text = String(actionPayload?.text || actionPayload?.content || serverMessage || "").trim();
        if (!text) throw new Error("Assistant returned an incomplete response. Please retry.");

        let insertPos = defaultInsertPos;
        const insertPayload = actionPayload?.insert;
        if (insertPayload && typeof insertPayload === "object") {
          const mode = String(insertPayload.mode || "").toLowerCase();
          if (mode === "top") {
            insertPos = 0;
          } else if (mode === "cursor") {
            insertPos = cursorPos;
          } else if ((mode === "before_heading" || mode === "after_heading") && String(insertPayload.heading_query || "").trim()) {
            const headingIdx = getHeadingIndexByQuery(editor, String(insertPayload.heading_query));
            const range = headingIdx != null ? getSectionRangeByHeadingIndex(editor, headingIdx) : null;
            if (range) {
              insertPos = mode === "before_heading" ? range.start : range.end;
            }
          }
        }

        let validationError = validateFreeformAssistantOutput(text);
        const applyLabel = typeof actionPayload?.apply_label === "string"
          ? actionPayload.apply_label
          : typeof actionPayload?.applyLabel === "string"
            ? actionPayload.applyLabel
            : undefined;

        if (!validationError && applyLabel === "Insert Summary") {
          validationError = validateBulletList(text, 5);
        }
      ...
      const assistantMessage: ChatMessage = {
        id: id(),
        role: "assistant",
        content: messageContent || serverMessage || "Suggestion ready.",
        createdAt: Date.now(),
        meta,
      };
  ...
  return (
    <div className={cn("flex h-full flex-col rounded-xl border border-border/40 bg-background shadow-lg overflow-hidden", className)}>
      <div className="shrink-0 px-4 py-3 border-b border-border/40 bg-muted/20">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-primary" />
            <div className="text-[13px] font-bold text-foreground">AI Assistant</div>
          </div>
        </div>
      </div>
          ...
          {messages.map((m, idx) => {
            ...
            return (
              <div key={m.id} className={cn("flex min-w-0 mb-4", isUser ? "justify-end" : "justify-start")}>
                <div
                  className={cn(
                    "min-w-0 rounded-2xl px-4 py-3 text-[13px] leading-relaxed whitespace-pre-wrap break-words transition-all duration-200 animate-in fade-in slide-in-from-bottom-2",
                    isUser
                      ? "max-w-[85%] rounded-tr-[4px] bg-primary text-primary-foreground shadow-sm"
                      : cn(
                        "bg-muted/30 border border-border/40 text-foreground shadow-sm rounded-tl-[4px]",
                ...
                >
                  {isUser && m.attachment && (
                    <div className="mb-2 block">
                      <div className="inline-flex items-center gap-1.5 rounded-md bg-background/20 px-2 py-1 text-[10px] font-medium text-primary-foreground/90 shadow-sm">
                        <FileText className="h-3 w-3" />
                        <span className="truncate max-w-[200px]">{m.attachment.label}</span>
                      </div>
                    </div>
                  )}
                  <div className={cn(isUser ? "text-primary-foreground/90" : "text-foreground/90")}>
                    {m.content}
                  </div>

                  {!isUser && meta?.kind === "table" && (
                    <div className="mt-2 overflow-hidden rounded-xl border border-slate-200/80 bg-white/90 shadow-sm dark:border-slate-700/70 dark:bg-slate-950/90">
                      <div className="flex items-center justify-between border-b border-slate-200/70 bg-slate-50/80 px-3 py-2 dark:border-slate-700/70 dark:bg-slate-900/80">
                        <div className="text-[11px] font-semibold uppercase tracking-[0.04em] text-slate-600 dark:text-slate-300">
                        ...
                        <span className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-medium text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">
                          AI preview
                        </span>
                      </div>
                      <div className="flex items-center gap-2 border-b border-slate-200/60 bg-white px-3 py-1.5 text-[10px] text-slate-600 dark:border-slate-700/70 dark:bg-slate-950 dark:text-slate-300">
                      ...
                      {tablePreview ? (
                        <div className="w-full max-w-full overflow-hidden bg-white dark:bg-slate-950">
                          <table className="w-full table-fixed text-[10px] leading-tight">
                            <thead className="bg-slate-50 text-slate-600 dark:bg-slate-900 dark:text-slate-300">
                              <tr>
                                {tablePreview.headers.map((h, i) => (
                                  <th key={`${m.id}-table-h-${i}`} className="max-w-0 border-b border-r border-slate-200 px-1.5 py-1 text-left font-semibold last:border-r-0 dark:border-slate-700">
                                    {h}
                                  </th>
                                ))}
                              </tr>
                            ...
                            <tbody>
                              {tablePreview.rows.map((row, ri) => (
                                <tr
                                  ...
                                  className={cn(
                                    "align-top",
                                    tablePreview.rowDiffKinds?.[ri] === "insert" && "bg-emerald-100/70 dark:bg-emerald-500/15",
                                    tablePreview.rowDiffKinds?.[ri] === "delete" && "bg-rose-100/70 dark:bg-rose-500/15",
                                    tablePreview.rowDiffKinds?.[ri] === "update" && "bg-amber-100/70 dark:bg-amber-500/15",
                                    !tablePreview.rowDiffKinds?.[ri] && "odd:bg-white even:bg-slate-50/35 dark:odd:bg-slate-950 dark:even:bg-slate-900/40"
                                  )}
                                >
                                  {tablePreview.headers.map((_, ci) => (
                                    <td
                                      ...
                                      className={cn(
                                        "border-b border-r border-slate-200/80 px-3 py-2 whitespace-normal break-words text-slate-700 last:border-r-0 dark:border-slate-700/80 dark:text-slate-200",
                                        "max-w-0 px-1.5 py-1 whitespace-nowrap overflow-hidden text-ellipsis",
                                        tablePreview.rowDiffKinds?.[ri] === "insert" && !tablePreview.cellDiffKinds?.[previewCellKey(ri, ci)] && "bg-emerald-100/80 text-emerald-950 dark:bg-emerald-500/20 dark:text-emerald-100",
                                        tablePreview.rowDiffKinds?.[ri] === "delete" && !tablePreview.cellDiffKinds?.[previewCellKey(ri, ci)] && "bg-rose-100/80 text-rose-950 dark:bg-rose-500/20 dark:text-rose-100",
                                        tablePreview.rowDiffKinds?.[ri] === "update" && !tablePreview.cellDiffKinds?.[previewCellKey(ri, ci)] && "bg-amber-100/80 text-amber-950 dark:bg-amber-500/20 dark:text-amber-100",
                                        tablePreview.cellDiffKinds?.[previewCellKey(ri, ci)] === "insert" && "bg-emerald-200/80 text-emerald-950 font-medium dark:bg-emerald-500/30 dark:text-emerald-50",
                                        tablePreview.cellDiffKinds?.[previewCellKey(ri, ci)] === "delete" && "bg-rose-200/80 text-rose-950 font-medium line-through dark:bg-rose-500/30 dark:text-rose-50",
                                        tablePreview.cellDiffKinds?.[previewCellKey(ri, ci)] === "update" && "bg-amber-200/80 text-amber-950 font-medium dark:bg-amber-500/30 dark:text-amber-50"
                                      )}
                                    >
                        ...
                        <div className="bg-white px-3 py-2 text-xs whitespace-pre-wrap font-mono text-slate-700 dark:bg-slate-950 dark:text-slate-200">
                          {(meta.rows || []).map((r) => r.join(" | ")).join("\n")}
                        </div>
                      ) : (
                        <div className="px-3 py-2 text-xs text-slate-500 dark:text-slate-400">No valid rows parsed.</div>
                      )}
                      {meta.parseError && (
                        <div className="border-t border-rose-200/60 bg-rose-50/70 px-3 py-2 text-[11px] text-rose-700 dark:border-rose-400/20 dark:bg-rose-950/40 dark:text-rose-200">{meta.parseError}</div>
                      )}
                    </div>
                  )}
                  ...
                  {!isUser && meta?.kind === "table_json" && (
                    <div className="mt-2 overflow-hidden rounded-xl border border-slate-200/80 bg-white/90 shadow-sm dark:border-slate-700/70 dark:bg-slate-950/90">
                      <div className="flex items-center justify-between border-b border-slate-200/70 bg-slate-50/80 px-3 py-2 dark:border-slate-700/70 dark:bg-slate-900/80">
                        <div className="text-[11px] font-semibold uppercase tracking-[0.04em] text-slate-600 dark:text-slate-300">Table instruction</div>
                        <span className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-medium text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">JSON</span>
                      </div>
                      <div className="flex items-center gap-2 border-b border-slate-200/60 bg-white px-3 py-1.5 text-[10px] text-slate-600 dark:border-slate-700/70 dark:bg-slate-950 dark:text-slate-300">
                        <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-500" />Added</span>
                        <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-amber-500" />Updated</span>
                      ...
                      {tableJsonPreview ? (
                            ...
                            <thead className="bg-slate-50 text-slate-600 dark:bg-slate-900 dark:text-slate-300">
                              <tr>
                                {tableJsonPreview.headers.map((h, i) => (
                                  <th key={`${m.id}-json-h-${i}`} className="max-w-0 border-b border-r border-slate-200 px-1.5 py-1 text-left font-semibold last:border-r-0 dark:border-slate-700">
                                    {h}
                                  </th>
                                ))}
                              </tr>
                            ...
                            <tbody>
                              {tableJsonPreview.rows.map((row, ri) => (
                                <tr
                                  ...
                                  className={cn(
                                    "align-top",
                                    tableJsonPreview.rowDiffKinds?.[ri] === "insert" && "bg-emerald-100/70 dark:bg-emerald-500/15",
                                    tableJsonPreview.rowDiffKinds?.[ri] === "delete" && "bg-rose-100/70 dark:bg-rose-500/15",
                                    tableJsonPreview.rowDiffKinds?.[ri] === "update" && "bg-amber-100/70 dark:bg-amber-500/15",
                                    !tableJsonPreview.rowDiffKinds?.[ri] && "odd:bg-white even:bg-slate-50/35 dark:odd:bg-slate-950 dark:even:bg-slate-900/40"
                                  )}
                                >
                                  {tableJsonPreview.headers.map((_, ci) => (
                                    <td
                                      ...
                                      className={cn(
                                        "border-b border-r border-slate-200/80 px-3 py-2 whitespace-normal break-words text-slate-700 last:border-r-0 dark:border-slate-700/80 dark:text-slate-200",
                                        "max-w-0 px-1.5 py-1 whitespace-nowrap overflow-hidden text-ellipsis",
                                        tableJsonPreview.rowDiffKinds?.[ri] === "insert" && !tableJsonPreview.cellDiffKinds?.[previewCellKey(ri, ci)] && "bg-emerald-100/80 text-emerald-950 dark:bg-emerald-500/20 dark:text-emerald-100",
                                        tableJsonPreview.rowDiffKinds?.[ri] === "delete" && !tableJsonPreview.cellDiffKinds?.[previewCellKey(ri, ci)] && "bg-rose-100/80 text-rose-950 dark:bg-rose-500/20 dark:text-rose-100",
                                        tableJsonPreview.rowDiffKinds?.[ri] === "update" && !tableJsonPreview.cellDiffKinds?.[previewCellKey(ri, ci)] && "bg-amber-100/80 text-amber-950 dark:bg-amber-500/20 dark:text-amber-100",
                                        tableJsonPreview.cellDiffKinds?.[previewCellKey(ri, ci)] === "insert" && "bg-emerald-200/80 text-emerald-950 font-medium dark:bg-emerald-500/30 dark:text-emerald-50",
                                        tableJsonPreview.cellDiffKinds?.[previewCellKey(ri, ci)] === "delete" && "bg-rose-200/80 text-rose-950 font-medium line-through dark:bg-rose-500/30 dark:text-rose-50",
                                        tableJsonPreview.cellDiffKinds?.[previewCellKey(ri, ci)] === "update" && "bg-amber-200/80 text-amber-950 font-medium dark:bg-amber-500/30 dark:text-amber-50"
                                      )}
                                    >
                                      {coerceCellValue(row?.[ci])}
                                    </td>
                        ...
                        <div className="px-3 py-2 text-xs text-slate-600 dark:text-slate-300">{meta.preview}</div>
                      ) : null}
                      {meta.instructionError && (
                        <div className="border-t border-rose-200/60 bg-rose-50/70 px-3 py-2 text-[11px] text-rose-700 dark:border-rose-400/20 dark:bg-rose-950/40 dark:text-rose-200">{meta.instructionError}</div>
                      )}
                    </div>
                  )}
      ...
      <div className="shrink-0 border-t border-border/10 p-4 bg-background/20">
        {(attached || manualFiles.length > 0) && (
          <div className="mb-3 flex flex-wrap items-center gap-2 px-1">
            {attached && (
              <div className="inline-flex items-center gap-2 rounded-lg border border-indigo-200/30 bg-indigo-50/50 px-2.5 py-1.5 text-[10px] font-semibold text-indigo-600/90 shadow-sm animate-in zoom-in-95 dark:border-indigo-400/20 dark:bg-indigo-500/10 dark:text-indigo-200">
                <div className="flex h-4 w-4 items-center justify-center rounded bg-indigo-100/40 dark:bg-indigo-500/15">
                  <FileText className="h-2.5 w-2.5" />
                </div>
                <span className="max-w-[200px] truncate">{attached.label}</span>
                <button
                  type="button"
                  className="ml-0.5 inline-flex h-4 w-4 items-center justify-center rounded-full transition-colors hover:bg-indigo-200/50 dark:hover:bg-indigo-400/15"
            ...
            {manualFiles.map((file) => (
              <div key={manualContextFileKey(file)} className="inline-flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-2.5 py-1.5 text-[11px] font-medium text-muted-foreground shadow-sm animate-in zoom-in-95">
                <div className="flex h-4 w-4 items-center justify-center rounded bg-muted">
                  <FileText className="h-2.5 w-2.5" />
                </div>
                <span className="rounded bg-background/80 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.08em] text-muted-foreground/80">
                  {manualContextSourceLabel(file.source)}
                </span>
                ...
                <button
                  ...
                  onClick={() => {
                    setManualFiles((prev) => prev.filter((f) => manualContextFileKey(f) !== manualContextFileKey(file)));
                  }}
                >
                  <X className="h-3 w-3" />
                </button>
        ...
        <div className="relative flex items-end gap-2 rounded-2xl border border-border/40 bg-background/50 p-1 shadow-sm backdrop-blur-sm transition-all focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/20">
          {mentionOpen && (
            <div
              ref={mentionMenuRef}
              className="absolute bottom-[calc(100%+8px)] left-0 z-20 min-w-[260px] rounded-xl border border-border/30 bg-background shadow-2xl p-1 animate-in fade-in slide-in-from-bottom-2 backdrop-blur-md"
            >
              <button
                type="button"
              ...
              >
                ...
                <div className="min-w-0">
                  <div>Attach editor doc</div>
                  <div className="text-[10px] font-medium text-muted-foreground">Use another editor document as context</div>
                </div>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <div className="min-w-0">
                  <div>Attach uploaded doc</div>
                  <div className="text-[10px] font-medium text-muted-foreground">Use a library document as context</div>
                </div>
              </button>
            </div>
          )}
          ...
          <div className="relative flex-1">
            ...
            <Button
              ...
              variant="ghost"
              size="icon"
              className="absolute right-1 top-2.5 h-8 w-8 rounded-lg text-muted-foreground transition-colors hover:bg-muted"
        ...
        <FinderPicker
          ...
          mode="doc"
          maxDocs={pickerMaxDocs}
          docSource={pickerSource}
          initialSelectedDocIds={currentPickerSelectedIds}
          onConfirm={({ docs }) => {
            setManualFiles((prev) => {
              const nextForSource = Array.isArray(docs)
                ? docs
                  .filter((doc): doc is StoredDocument => Boolean(doc?.id))
                  .map((doc) => normalizePickedDoc(doc, pickerSource))
                : [];
              const combined = [
                ...prev.filter((file) => file.source !== pickerSource),
                ...nextForSource,
              ];
              const deduped: ManualContextFile[] = [];
              const seen = new Set<string>();
              for (const file of combined) {
                const key = manualContextFileKey(file);
                if (seen.has(key)) continue;
                seen.add(key);
                deduped.push(file);
