...
import { EditorTextInputDialog } from "@/components/editor/editor-text-input-dialog";
...
import { ApprovalCommentsExtension, type ApprovalCommentAnchor } from "@/components/editor/comments/approval-comments-extension";
...
class NotionLikeTableView extends TableView {
  ...
  constructor(node: ProseMirrorNode, cellMinWidth: number, view: EditorView) {
    ...
    bottomButton.addEventListener("keydown", this.handleBottomAddKeyDown);
    ...
    rightButton.addEventListener("keydown", this.handleRightAddKeyDown);
  ...
  private handleBottomAddKeyDown = (event: KeyboardEvent) => {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    event.stopPropagation();
    this.commitResize(1, 0);
  };

  private handleRightAddKeyDown = (event: KeyboardEvent) => {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    event.stopPropagation();
    this.commitResize(0, 1);
  };
  ...
  destroy() {
    ...
    this.bottomFooterButton.removeEventListener("keydown", this.handleBottomAddKeyDown);
    this.rightAddButton.removeEventListener("pointerdown", this.handleRightAddPointerDown);
    this.rightAddButton.removeEventListener("keydown", this.handleRightAddKeyDown);
...
type Props = {
  ...
  onSelectionRangeChange?: (selection: { from: number; to: number; quote: string } | null) => void;
  placeholder?: string;
  className?: string;
  ...
  approvalComments?: ApprovalCommentAnchor[];
  activeApprovalCommentId?: string | null;
  onApprovalCommentSelect?: (commentId: string | null) => void;
};
...
type SlashCommand = {
  ...
  run: (props: { editor: Editor; range: { from: number; to: number } }) => void;
};
...
function buildSlashTableNode(): JSONContent {
  return {
    type: "table",
...
type BlockDropIndicatorState = {
  top: number;
  left: number;
  width: number;
};

function getBlockDropIndicatorState(
  root: HTMLElement,
  shell: HTMLElement,
  targetInsertIndex: number,
): BlockDropIndicatorState | null {
  ...
  if (childCount === 0) {
    return {
      top: Math.max(0, rootRect.top - shellRect.top + 8),
      left: Math.max(0, rootRect.left - shellRect.left + 24),
      width: Math.max(120, rootRect.width - 48),
    };
  }

  let top = 0;
  if (targetInsertIndex <= 0) {
    const first = root.children.item(0);
    if (!(first instanceof HTMLElement)) return null;
    top = first.getBoundingClientRect().top - shellRect.top;
  } else if (targetInsertIndex >= childCount) {
    const last = root.children.item(childCount - 1);
    if (!(last instanceof HTMLElement)) return null;
    top = last.getBoundingClientRect().bottom - shellRect.top;
  } else {
    const target = root.children.item(targetInsertIndex);
    if (!(target instanceof HTMLElement)) return null;
    top = target.getBoundingClientRect().top - shellRect.top;
  }
...
const SLASH_COMMANDS: SlashCommand[] = [
  ...
  },
  {
    ...
    run: ({ editor, range }) => {
      editor.commands.insertContentAt(
        range,
        [buildSlashTableNode(), { type: "paragraph" }],
        { updateSelection: true }
      );
    },
  },
  ...
  },
  {
    ...
    icon: Code,
    run: ({ editor, range }) => editor.chain().focus().deleteRange(range).toggleCode().run(),
  },
  {
    id: "clear-formatting",
    ...
    icon: Text,
    run: ({ editor, range }) => editor.chain().focus().deleteRange(range).unsetAllMarks().clearNodes().run(),
  },
];
...
const LINK_SLASH_COMMAND_ID = "link";
...
function getSlashContext(editor: Editor): SlashContext | null {
  ...
  // Only open the slash menu when "/" is the very first character in the
  // text block (Notion-style). This avoids triggering mid-word or mid-sentence.
  if (slashIndex !== 0) return null;
...
export function TipTapEditor({
  ...
  approvalComments = [],
  activeApprovalCommentId = null,
  onApprovalCommentSelect,
}: Props) {
  ...
  const onSelectionRangeChangeRef = React.useRef(onSelectionRangeChange);
  const lastPublishedSelectionRef = React.useRef<string | null>(null);
  ...
  const [blockDropIndicator, setBlockDropIndicator] = React.useState<BlockDropIndicatorState | null>(null);
  ...
  const [calloutEmojiDialog, setCalloutEmojiDialog] = React.useState<{
    open: boolean;
    value: string;
  ...
  const slashMenuItemsRef = React.useRef<SlashCommand[]>([]);
  const slashSelectedIndexRef = React.useRef(0);
  const slashRangeRef = React.useRef<{ from: number; to: number } | null>(null);
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      ApprovalCommentsExtension.configure({
        getComments: () => approvalComments,
        getActiveCommentId: () => activeApprovalCommentId,
        onSelectCommentId: (commentId) => onApprovalCommentSelect?.(commentId),
      }),
  ...
  React.useEffect(() => {
    ...
    const publishSelection = () => {
      const publish = onSelectionRangeChangeRef.current;
      if (!publish) return;

      const { from, to, empty } = editor.state.selection;
      if (empty || to <= from) {
        if (lastPublishedSelectionRef.current === "__empty__") return;
        lastPublishedSelectionRef.current = "__empty__";
        publish(null);
        return;
      }
      ...
      const nextKey = `${from}:${to}:${quote}`;
      if (lastPublishedSelectionRef.current === nextKey) return;
      lastPublishedSelectionRef.current = nextKey;
      publish({ from, to, quote });
    };
  ...
  const openLinkEditor = React.useCallback(() => {
    ...
    setLinkEditorValue(currentHref);
    setLinkEditorOpen(true);
  }, [editor]);

  const submitLinkEditor = React.useCallback(() => {
    if (!editor) return;
    const nextHref = linkEditorValue.trim();
  ...
  const removeLinkFromEditor = React.useCallback(() => {
    if (!editor) return;
  ...
  const getActiveSlashCommand = React.useCallback(() => {
    const items = slashMenuItemsRef.current;
    if (!items.length) return null;
    return items[slashSelectedIndexRef.current] || items[0] || null;
  }, []);

  const runSlashCommand = React.useCallback((command: SlashCommand) => {
    if (!editor) return;
    const context = slashRangeRef.current ?? getSlashContext(editor);
    if (!context) return;
    const range = { from: context.from, to: context.to };
    command.run({ editor, range });
    if (command.id === LINK_SLASH_COMMAND_ID) {
      openLinkEditor();
    }
    rememberSlashCommand(command.id);
    closeSlashMenu();
  }, [closeSlashMenu, editor, openLinkEditor, rememberSlashCommand]);
  ...
  React.useEffect(() => {
    ...
    const updateSlashMenu = () => {
      ...
      slashRangeRef.current = { from: context.from, to: context.to };
    ...
    // Bug fix #4: scope scroll listener to the editor shell instead of
    // capturing every scroll in the document. This prevents unnecessary
    // recomputes (and menu flicker) when unrelated elements scroll.
    const scrollContainer = editorShellRef.current ?? window;

    updateSlashMenu();
    editor.on("selectionUpdate", updateSlashMenu);
    editor.on("transaction", updateSlashMenu);
    editor.on("blur", handleEditorBlur);
    scrollContainer.addEventListener("scroll", updateSlashMenu);
    ...
    return () => {
      ...
      scrollContainer.removeEventListener("scroll", updateSlashMenu);
      window.removeEventListener("resize", updateSlashMenu);
    };
  }, [closeSlashMenu, editable, editor]);

  // Bug fix #3: single effect handles index reset on query change — the
  // previous duplicate effect that clamped on slashMenuItems.length change
  // caused double setState calls and stale-order flickers.
  React.useEffect(() => {
    setSlashSelectedIndex(0);
  }, [slashQuery]);
  ...
  React.useEffect(() => {
    ...
    // IMPORTANT: We register this handler on the editor shell (a parent of
    // editor.view.dom) in the CAPTURE phase so it fires BEFORE ProseMirror's
    // own keydown handler. If we used the default bubble phase on the same
    // element, ProseMirror would process Enter first (splitting the block and
    // dispatching a transaction), which synchronously clears the slash context
    // and range — making it impossible for our handler to execute the command.
    const handleKeyDown = (event: KeyboardEvent) => {
      // Only intercept when the event target is inside the editor
      ...
      if (event.key === "ArrowDown") {
        ...
        event.stopPropagation();
        const items = slashMenuItemsRef.current;
        setSlashSelectedIndex((prev) => (items.length ? (prev + 1) % items.length : 0));
        return;
      }
      ...
      if (event.key === "Enter") {
        event.preventDefault();
        event.stopPropagation();
        const command = getActiveSlashCommand();
    ...
    const shell = editorShellRef.current;
    if (!shell) return;
    // Capture phase on parent ensures we fire before ProseMirror's handler
    shell.addEventListener("keydown", handleKeyDown, true);
    return () => shell.removeEventListener("keydown", handleKeyDown, true);
  }, [closeSlashMenu, editor, getActiveSlashCommand, runSlashCommand, slashOpen]);

  // Bug fix #5: close the slash menu when the user clicks outside the panel.
  React.useEffect(() => {
    if (!slashOpen) return;
  ...
  const moveActiveBlocks = React.useCallback((direction: "up" | "down") => {
    if (!editor) return false;

    const active = normalizeTopLevelIndices(resolveActiveBlockIndices(), editor.state.doc.childCount);
    if (active.length === 0) return false;

    const targetInsertIndex = direction === "up"
      ? active[0] - 1
      : active[active.length - 1] + 2;

    const nextIndices = moveBlocksToInsertIndex(editor, active, targetInsertIndex);
    if (!nextIndices || nextIndices.length === 0) return false;
    ...
    window.requestAnimationFrame(() => {
      const root = editor.view.dom as HTMLElement;
      const hovered = root.children.item(nextIndices[0]);
      if (hovered instanceof HTMLElement) {
        setHoverFromElement(hovered, { keepMenuOpen: false });
      }
    });
    ...
    return true;
  }, [editor, resolveActiveBlockIndices, setHoverFromElement]);
  ...
  const openCalloutEmojiDialog = React.useCallback(() => {
    if (!editor) return;
    ...
    setCalloutEmojiDialog({
      open: true,
      value: String(firstCallout.node.attrs?.emoji || "").trim(),
      targetIndices: active,
    });
    setHoverMenuOpen(false);
    setHoverColorPanel(null);
  }, [editor, resolveActiveBlockIndices]);

  const submitCalloutEmojiDialog = React.useCallback(() => {
    if (!editor) return;

    const active = normalizeTopLevelIndices(calloutEmojiDialog.targetIndices, editor.state.doc.childCount);
    if (active.length === 0) {
      setCalloutEmojiDialog({ open: false, value: "", targetIndices: [] });
      return;
    }

    const nextEmoji = calloutEmojiDialog.value.trim();
    const tr = editor.state.tr;
    let updatedCount = 0;
    ...
    setCalloutEmojiDialog({ open: false, value: "", targetIndices: [] });
  }, [calloutEmojiDialog.targetIndices, calloutEmojiDialog.value, editor]);
  ...
  const runBlockUiAction = React.useCallback((
    event: React.MouseEvent<HTMLElement> | React.KeyboardEvent<HTMLElement>,
    action: () => void,
    options?: { stopPropagation?: boolean; selectHoverBlock?: boolean },
  ) => {
    if ("button" in event && event.type === "mousedown" && event.button !== 0) return;
    if ("key" in event && event.type === "keydown" && event.key !== "Enter" && event.key !== " ") return;

    event.preventDefault();
    if (options?.stopPropagation) {
      event.stopPropagation();
    }
    if (options?.selectHoverBlock) {
      const shiftKey = "shiftKey" in event ? Boolean(event.shiftKey) : false;
      selectHoverBlockFromGutter({ shiftKey });
    }
    action();
  }, [selectHoverBlockFromGutter]);

  React.useEffect(() => {
    if (!editor || !editable) return;

    const onEditorKeyDown = (event: KeyboardEvent) => {
      const isLinkShortcut = (event.metaKey || event.ctrlKey) && !event.shiftKey && event.key.toLowerCase() === "k";
      if (isLinkShortcut) {
        event.preventDefault();
        openLinkEditor();
        return;
  ...
  }, [deleteActiveBlocks, duplicateActiveBlocks, editable, editor, openLinkEditor]);
  ...
  const toggleHoverMenuFromGutter = React.useCallback(() => {
  ...
  const startDragBlock = React.useCallback((
    ...
    const onMove = (moveEvent: MouseEvent) => {
      ...
      frame = window.requestAnimationFrame(() => {
        ...
        const isNoopDrop = targetInsertIndex >= currentIndices[0]
          && targetInsertIndex <= currentIndices[currentIndices.length - 1] + 1;
        if (isNoopDrop) {
          dropInsertIndex = null;
          if (lastPreviewKey !== "noop") {
            lastPreviewKey = "noop";
            setBlockDropIndicator(null);
          }
          return;
        ...
        dropInsertIndex = targetInsertIndex;
        const previewKey = String(targetInsertIndex);
        if (previewKey === lastPreviewKey) return;
        lastPreviewKey = previewKey;

        if (!shell) return;
        setBlockDropIndicator(getBlockDropIndicatorState(root, shell, targetInsertIndex));
      });
    };
    ...
    const onUp = () => {
      ...
      if (dragging) {
        ...
        if (dropInsertIndex !== null) {
          const nextIndices = moveBlocksToInsertIndex(editor, currentIndices, dropInsertIndex);
          if (nextIndices && nextIndices.length > 0) {
            ...
            if (firstMovedEl instanceof HTMLElement) {
              setHoverFromElement(firstMovedEl, { keepMenuOpen: false });
            }
          }
        }
        editor.view.focus();
  ...
  const isPlainSurface = surfaceVariant === "plain";

  return (
    <div
      className={cn(
        isPlainSurface
          ? "editor-surface-plain bg-transparent border-0 rounded-none shadow-none"
          : "rounded-lg border bg-card/50 border-border/40",
        className
      )}
    >
      {showToolbar && (
        <div
          className={cn(
            isPlainSurface
              ? "border-b border-border/20 bg-transparent px-0 py-2"
              : "border-b border-border/40 bg-muted/10 px-2 py-2",
            stickyToolbar && (isPlainSurface
              ? "sticky top-[var(--editor-toolbar-sticky-top)] z-30 bg-background backdrop-blur"
              : "sticky top-[var(--editor-toolbar-sticky-top)] z-30 bg-background backdrop-blur")
          )}
          style={toolbarStickyStyle}
        >
          <TipTapToolbar editor={editor} onRequestLinkEdit={openLinkEditor} />
        </div>
      )}
      {editable && (
        <div className={cn(
          "border-b text-[11px] text-muted-foreground",
          isPlainSurface
            ? "border-border/15 bg-transparent px-0 py-1.5"
            : "border-border/30 bg-background px-4 py-1.5"
        )}>
      ...
      <div
        ...
        className={cn(
          "relative",
          isPlainSurface ? "px-0 py-6" : "px-4 py-4"
        )}
      >
        {editor && showBubbleMenu && editable && (
          <BubbleMenu
            editor={editor}
          >
            <TipTapBubbleMenu editor={editor} onRequestLinkEdit={openLinkEditor} />
          </BubbleMenu>
        )}
        ...
        {blockDropIndicator && (
          <div
            ...
            aria-hidden="true"
            style={{
              top: blockDropIndicator.top - 1,
              left: blockDropIndicator.left,
              width: blockDropIndicator.width,
            }}
          >
            <div className="h-0.5 rounded-full bg-primary/80 shadow-[0_0_0_1px_rgba(255,255,255,0.7)]" />
          </div>
        )}
        ...
        {editable && hoverBlock && !slashOpen && (
          <>
            ...
            >
              <div
                className={cn(
                  "flex items-center gap-1 rounded-md border border-border/70 bg-background px-1 py-0.5 shadow-sm",
              ...
              >
                <button
                  ...
                  onMouseDown={(event) => runBlockUiAction(event, insertBlockBelow)}
                  onKeyDown={(event) => runBlockUiAction(event, insertBlockBelow)}
                ...
                <button
                  ...
                  className={cn(
                    ...
                    hoverMenuOpen && "bg-muted text-foreground"
                  )}
                  onMouseDown={(event) => runBlockUiAction(
                    event,
                    toggleHoverMenuFromGutter,
                    { stopPropagation: true, selectHoverBlock: true },
                  )}
                  onKeyDown={(event) => runBlockUiAction(
                    event,
                    toggleHoverMenuFromGutter,
                    { stopPropagation: true, selectHoverBlock: true },
                  )}
                ...
                <button
                  ...
                  title="Drag to move selected blocks"
                >
            ...
            {hoverMenuOpen && (
              <div
                className="absolute left-0 z-40 min-w-[250px] overflow-y-auto rounded-md border border-border/70 bg-background p-1 shadow-lg"
                data-editor-block-ui="true"
                onMouseEnter={onHoverUiEnter}
                onMouseLeave={onHoverUiLeave}
                style={{
              ...
              >
                <button
                  ...
                  onMouseDown={(event) => runBlockUiAction(event, () => moveActiveBlocks("up"))}
                  onKeyDown={(event) => runBlockUiAction(event, () => moveActiveBlocks("up"))}
                >
                ...
                <button
                  ...
                  onMouseDown={(event) => runBlockUiAction(event, () => moveActiveBlocks("down"))}
                  onKeyDown={(event) => runBlockUiAction(event, () => moveActiveBlocks("down"))}
                >
                ...
                <button
                  type="button"
                  className="flex h-7 w-full items-center gap-2 rounded-sm px-2 text-xs text-muted-foreground transition-colors hover:bg-muted"
                  onMouseDown={(event) => runBlockUiAction(event, duplicateActiveBlocks)}
                  onKeyDown={(event) => runBlockUiAction(event, duplicateActiveBlocks)}
                >
                ...
                <button
                  type="button"
                  className="mt-1 flex h-7 w-full items-center gap-2 rounded-sm px-2 text-xs text-muted-foreground transition-colors hover:bg-muted"
                  onMouseDown={(event) => runBlockUiAction(event, () => convertActiveBlocksToCallout("tip"))}
                  onKeyDown={(event) => runBlockUiAction(event, () => convertActiveBlocksToCallout("tip"))}
                >
                ...
                <button
                  ...
                  onMouseDown={(event) => runBlockUiAction(event, deleteHoverBlock)}
                  onKeyDown={(event) => runBlockUiAction(event, deleteHoverBlock)}
                >
                ...
                <button
                  ...
                  onMouseDown={(event) => runBlockUiAction(event, () => setHoverColorPanel("text"))}
                  onKeyDown={(event) => runBlockUiAction(event, () => setHoverColorPanel("text"))}
                >
                ...
                <button
                  ...
                  onMouseDown={(event) => runBlockUiAction(event, () => setHoverColorPanel("background"))}
                  onKeyDown={(event) => runBlockUiAction(event, () => setHoverColorPanel("background"))}
                >
                ...
                {hoverBlock.nodeType === "callout" && (
                  <>
                    ...
                    <button
                      type="button"
                      className="flex h-7 w-full items-center gap-2 rounded-sm px-2 text-xs text-muted-foreground transition-colors hover:bg-muted"
                      onMouseDown={(event) => runBlockUiAction(event, openCalloutEmojiDialog)}
                      onKeyDown={(event) => runBlockUiAction(event, openCalloutEmojiDialog)}
                    >
                    ...
                    <div className="mt-1 grid grid-cols-2 gap-1 px-1">
                      {CALLOUT_TONE_META.map((option) => (
                        <button
                          ...
                          onMouseDown={(event) => runBlockUiAction(event, () => convertActiveBlocksToCallout(option.tone))}
                          onKeyDown={(event) => runBlockUiAction(event, () => convertActiveBlocksToCallout(option.tone))}
                        >
                          <span
            ...
            {hoverMenuOpen && hoverColorPanel && hoverColorPanelPosition && (
              <div
                className="absolute left-0 z-50 w-[230px] overflow-y-auto rounded-md border border-border/70 bg-background p-1 shadow-lg"
                data-editor-block-ui="true"
                onMouseEnter={onHoverUiEnter}
                onMouseLeave={onHoverUiLeave}
                style={{
                ...
                <div className="space-y-1 p-1">
                  {activeHoverColorOptions.map((option) => (
                    <button
                      ...
                      type="button"
                      className="flex h-7 w-full items-center gap-2 rounded-sm px-2 text-xs text-muted-foreground transition-colors hover:bg-muted"
                      onMouseDown={(event) => runBlockUiAction(event, () => {
                      ...
                      onKeyDown={(event) => runBlockUiAction(event, () => {
                        if (hoverColorPanel === "background") {
                          applyBackgroundColorToActiveBlocks(option.color);
                          return;
                        }
        ...
        {slashOpen && editable && (
          <div
            className="fixed z-50 w-[360px] rounded-xl border border-border/60 bg-background shadow-2xl"
          ...
          >
            <div className="px-3 py-2 border-b border-border/50 space-y-2">
              ...
              <div className="rounded-md border border-border/60 bg-muted/50 px-2 py-1.5 text-xs text-muted-foreground">
                {slashQuery ? `/${slashQuery}` : "Type after / in the editor"}
              </div>
            </div>
        ...
        <EditorTextInputDialog
          open={linkEditorOpen}
          onOpenChange={(open) => setLinkEditorOpen(open)}
          title="Edit link"
          description="Add a URL for the current selection. Leave it blank to remove the link."
          value={linkEditorValue}
          onValueChange={setLinkEditorValue}
          placeholder="https://example.com"
          submitLabel="Apply link"
          secondaryLabel="Remove link"
          onSecondaryAction={removeLinkFromEditor}
          onSubmit={submitLinkEditor}
        />

        <EditorTextInputDialog
          open={calloutEmojiDialog.open}
          onOpenChange={(open) => {
            if (!open) {
              setCalloutEmojiDialog({ open: false, value: "", targetIndices: [] });
            }
          }}
          title="Edit callout emoji"
          description="Choose the emoji shown in the selected callout."
          value={calloutEmojiDialog.value}
          onValueChange={(value) => setCalloutEmojiDialog((prev) => ({ ...prev, value }))}
          placeholder="e.g. 💡"
          submitLabel="Apply emoji"
          onSubmit={submitCalloutEmojiDialog}
        />
      </div>
    </div>
  );
