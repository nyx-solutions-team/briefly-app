...
type ApprovalCommentsOptions = {
  ...
  onSelectCommentId?: (commentId: string | null) => void;
};
...
export const ApprovalCommentsExtension = Extension.create<ApprovalCommentsOptions>({
  ...
  addProseMirrorPlugins() {
    ...
    const onSelectCommentId = this.options.onSelectCommentId;
      ...
      new Plugin({
        ...
        props: {
          ...
          handleClick: (_view, _pos, event) => {
            ...
            const commentEl = target.closest("[data-approval-comment-id]");
            if (!(commentEl instanceof HTMLElement)) return false;
            const commentId = String(commentEl.getAttribute("data-approval-comment-id") || "").trim();
