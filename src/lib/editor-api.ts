...
export type CreateEditorDocBody = {
  ...
  templateProvenance?: {
    ...
    template_scope_type: 'global' | 'org' | 'department';
    template_department_id?: string | null;
    template_origin?: 'editor' | 'chat' | 'api' | 'import' | 'system';
  };
};
...
export function toEditorDocFilename(title: string): string {
...
export async function createEditorDocShell(body: CreateEditorDocShellBody): Promise<CreateEditorDocShellResponse> {
  ...
  const filename = toEditorDocFilename(title);
