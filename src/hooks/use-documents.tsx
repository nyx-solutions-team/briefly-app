...
export function DocumentsProvider({ children }: { children: React.ReactNode }) {
  ...
  const shouldAutoLoad =
    ...
    pathname !== '/chatnew' &&
    !(pathname?.startsWith('/chatnew/')) &&
    pathname !== '/chat-workbench' &&
    !(pathname?.startsWith('/chat-workbench/')) &&
    pathname !== '/chatworkbench' &&
    !(pathname?.startsWith('/chatworkbench/'));
  ...
  }, [shouldAutoLoad]); // Remove refresh dependency to prevent infinite loops
  ...
  useEffect(() => {
    const off = onApiContextChange(() => {
      ...
      const orgId = getOrgId();
      if (shouldAutoLoad && orgId && !loadingRef.current) {
        void refresh();
      }
    });
  ...
  const deleteFolder = useCallback(async (path: string[], mode: 'move_to_root' | 'move_to_recycle_bin' = 'move_to_root') => {
    ...
    try {
      ...
      if (mode === 'move_to_root' && result.documentsHandled > 0) {
        void refresh();
      } else if (mode === 'move_to_recycle_bin') {
        // Remove recycled documents from local state across the entire folder subtree.
        setDocuments(prev => prev.filter(d => {
          const folderPath = d.folderPath || [];
          if (folderPath.length < path.length) return true;
          return !path.every((seg, i) => folderPath[i] === seg);
        }));
      }
