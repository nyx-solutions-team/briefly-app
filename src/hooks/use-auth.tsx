...
const PUBLIC_AUTH_SAFE_PATH_PREFIXES = ['/signin', '/no-access', '/ip-blocked', '/share', '/folder-share'];

function isPublicAuthSafePath(pathname: string | null | undefined) {
  if (!pathname) return false;
  return PUBLIC_AUTH_SAFE_PATH_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`));
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const authInitStartedRef = React.useRef(false);
  const router = useRouter();

  const clearLocalAuthState = useCallback(({ redirect = true, resetInit = true }: { redirect?: boolean; resetInit?: boolean } = {}) => {
    setUser(null);
  ...
  const applyAuthenticatedState = useCallback((session: { user?: { email?: string | null; id?: string | null } } | null, bootstrap: BootstrapData, preferredOrgId?: string | null) => {
    const now = Date.now();
    ...
    const preferred = String(preferredOrgId || bootstrap.selectedOrgId || '').trim();
    const activeOrgId =
      (preferred ? activeOrgs.find((o: any) => String(o.orgId) === preferred)?.orgId : null)
      || activeOrgs[0]?.orgId
      || '';

    setApiContext({ orgId: activeOrgId });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!token) {
          clearLocalAuthState({ redirect: true, resetInit: false });
          if (mounted) setIsLoading(false);
          return;
        }

        const bootstrap = await fetchBootstrapData<BootstrapData>({
          accessToken: token,
          orgId: getApiContext().orgId || undefined,
        });
        if (!mounted) return; // Component unmounted

        applyAuthenticatedState(sess.session, bootstrap, getApiContext().orgId || undefined);
      } catch (error) {
  ...
  }, [applyAuthenticatedState, clearLocalAuthState]);
  ...
  const signIn = useCallback(async ({ username, password }: { username: string; password: string; email?: string }) => {
    ...
    try {
      ...
      const bootstrap = await fetchBootstrapData<BootstrapData>({ accessToken: data.session.access_token });
      const currentOrgId = applyAuthenticatedState(data.session, bootstrap);
      ...
      if (currentOrgId) {
        ...
        if (timeSinceLastLogin > 60000) { // 60 seconds
          ...
          fetch(`${base}/orgs/${currentOrgId}/audit/login`, {
            method: 'POST',
  ...
  }, [applyAuthenticatedState, lastLoginLoggedAt]);

  const signOut = useCallback(() => {
    clearLocalAuthState({ redirect: false });
    try {
  ...
  const refreshPermissionsForCurrentOrg = useCallback(async () => {
    ...
    try {
      ...
      const freshBootstrap = await fetchBootstrapData<BootstrapData>({
  ...
  useEffect(() => {
    const {
      ...
      if (event === 'SIGNED_IN' && user && bootstrapData) {
        resetAuthFailureState();
        return;
      }

      try {
        const currentOrgId = getApiContext().orgId || undefined;
        const freshBootstrap = await fetchBootstrapData<BootstrapData>({
          accessToken: session.access_token,
          orgId: currentOrgId,
          force: true,
        });
        applyAuthenticatedState(session, freshBootstrap, currentOrgId);
      } catch (error) {
  ...
  }, [applyAuthenticatedState, bootstrapData, clearLocalAuthState, user]);
